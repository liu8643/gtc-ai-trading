...
def build_bull_bear_path(data: dict) -> str:
    ...
    if fibo_dir == "下降波":
        return "\n".join([
            "【反彈 / 續弱路徑】",
            "◎ 反彈路徑：",
            f"→ 反彈路徑①：先觀察 {support} 是否止穩",
            f"→ 反彈路徑②：若站回 {support} 並轉強，再挑戰 {resistance}",
            f"→ 反彈路徑③：只有有效站上 {resistance}，弱勢反彈才有機會升級",
            "",
            "◎ 續弱路徑：",
            f"→ 續弱路徑①：若再失守 {support}",
            "→ 續弱路徑②：空方仍掌握節奏，先以保守控管為主",
            f"→ 續弱路徑③：若續弱延伸，下一觀察位看 {next_target}",
            "",
...
def evaluate_trade_state(close, prev_close, open_price, support, resistance, change_pct,
    ...
    too_near_resistance = close >= resistance * 0.997 if resistance else False
    ...
    if (
        ...
        if too_near_resistance:
            return "強勢追蹤", "拉回不破可偏多", "strong"
        return "強勢追蹤", "拉回布局", "strong"

    if (
        trend_score >= 72 and intraday_score >= 58 and score >= 70 and
        change_pct >= 0.8 and above_open and above_prev and orderbook_bias != "賣盤偏強"
    ):
        return "整理偏多", "拉回布局", "bullish"

    if (
        trend_score >= 66 and intraday_score >= 52 and score >= 62 and
        change_pct >= 0.2 and above_open
    ):
...
def classify_leader_stage(data: dict) -> str:
    close = data.get("close", 0)
    resistance = data.get("resistance", 0)
    near_resistance = close >= resistance * 0.997 if resistance else False
    ...
    if (
        score >= 82 and trend >= 78 and intra >= 68 and
...
def calc_trade_plan(data: dict) -> dict:
    ...
    if state == "weak":
        return {
            "entry_low": 0.0,
            "entry_high": 0.0,
            "stop_loss": round_price(support * 0.968) if support else 0.0,
            "target_price": round_price(resistance) if resistance else 0.0,
            "rr": 0.0,
            "observe_support": round_price(support) if support else 0.0,
            "observe_resistance": round_price(resistance) if resistance else 0.0,
        }
...
def build_trade_scripts(data: dict) -> dict:
    ...
    if fibo_dir == "下降波":
        return {
            "script_a": f"劇本A（反彈試壓）: 若先在 {support} 附近止穩，可觀察是否有機會反彈挑戰 {resistance}",
            "script_b": f"劇本B（續弱下修）: 若跌回 {support} 下方，代表弱勢未改，先保留現金與風險控管",
            "script_c": f"劇本C（止穩轉強）: 只有重新站上 {resistance} 並量價轉強，才考慮由弱勢反彈升級為偏多",
        }
...
def build_market_overview(results: list[dict]) -> str:
    ...
    if strong >= max(2, total * 0.35):
        market = "盤勢：偏多到強勢"
        strategy = "今日策略：只做強勢/主升股拉回，不追弱勢反彈"
    elif weak >= max(3, total * 0.5):
        market = "盤勢：偏弱震盪"
        strategy = "今日策略：優先防守，弱勢股不抄底，只觀察支撐是否止穩"
    elif bullish + strong >= max(3, total * 0.4):
        market = "盤勢：震盪偏多"
        strategy = "今日策略：挑偏多與整理偏多股，等支撐承接或突破確認"
    else:
        market = "盤勢：區間震盪"
        strategy = "今日策略：區間操作為主，靠支撐觀察、靠壓力保守"
...
class GTCProApp:
    ...
    def _build_advice_lines(self, target: dict):
        ...
        weak_mode = target.get("state_bucket") == "weak"
        entry_text = (
            "不建議主動進場"
            if weak_mode else
            f"{target['entry_low']} ~ {target['entry_high']}"
        )
        stop_text = target.get('stop_loss', 0)
        target_price_text = target.get('target_price', target['resistance'])
        ...
        if weak_mode:
            lines.extend([
                f"觀察支撐：{target.get('observe_support', target['support'])}",
                f"反彈壓力：{target.get('observe_resistance', target['resistance'])}",
                "策略重點：先等止穩，不建議主動摸底",
            ])
        else:
            lines.extend([
                f"停損點：{stop_text}",
                f"第一目標：{target_price_text}",
                f"風險報酬比：{rr_text}",
            ])
