...
# AI投資組合使用 pool_config.json 控制勾選、配額與權重，不回寫/取代 TOP20。
AI_PORTFOLIO_CONFIG_PATH = RUNTIME_DIR / "pool_config.json"
DEFAULT_AI_PORTFOLIO_CONFIG = {
    "市場主流池": {"enabled": True, "quota": 5, "weight": 1.20},
    "未來科技池": {"enabled": True, "quota": 5, "weight": 1.15},
    "營收成長池": {"enabled": True, "quota": 3, "weight": 1.10},
    "價值池": {"enabled": True, "quota": 3, "weight": 1.00},
    "波段攻擊池": {"enabled": True, "quota": 3, "weight": 1.15},
    "老師低接池": {"enabled": True, "quota": 2, "weight": 1.05},
    "ETF防守池": {"enabled": False, "quota": 3, "weight": 0.80},
    "排除風險池": {"enabled": True, "quota": 20, "weight": 0.00},
}
AI_PORTFOLIO_FUTURE_TECH_KEYWORDS = [
...
# CLASSIFICATION_TPEX_JSON_FALLBACK_R2_20260604：分類來源改為本機 data/classification 優先；官網只作新版 OpenAPI JSON fallback。
# 讀取順序：1) EXE 旁 RUNTIME_DIR/data/classification 2) 程式內 BASE_DIR/data/classification 3) OpenAPI JSON。
# OpenAPI 失敗僅記錄 warning，不阻斷開機；JSON 解析失敗不再改用 CSV/cp950 硬讀。
...
CLASSIFICATION_DOWNLOAD_URL_TPEX_R = "https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap03_R"
...
CLASSIFICATION_CACHE_CSV_TPEX_R = CLASSIFICATION_CACHE_DIR / "台股官方產業分類_興櫃.csv"
...
CLASSIFICATION_OFFICIAL_CACHE = {
    ...
    "興櫃": {"df": None, "path": "", "mtime": 0.0, "rows": 0},
...
CLASSIFICATION_DOWNLOAD_SOURCES = {
    ...
    "興櫃": {
        "url": CLASSIFICATION_DOWNLOAD_URL_TPEX_R,
        "cache_path": CLASSIFICATION_CACHE_CSV_TPEX_R,
...
def safe_read_json_auto(path: Path) -> pd.DataFrame:
    """嚴格讀取官方 OpenAPI JSON 陣列；失敗時不 fallback 到 CSV/cp950。"""
    path = Path(path)
    last_error = None
...
def safe_read_classification_source(path: Path) -> pd.DataFrame:
    """分類來源統一讀取：本機 CSV 走 CSV parser；JSON 來源嚴格走 JSON parser。"""
    path = Path(path)
...
def _download_single_classification_csv(market: str, force_refresh: bool = False, log_cb=None) -> Optional[Path]:
    ...
    只在本機 data/classification 與 BASE_DIR/data/classification 都找不到時執行。
    官方 API 正常回傳 JSON；若 JSON 解析失敗，直接視為 fallback 失敗，不再當 CSV/cp950 硬讀。
    """
        ...
        try:
            ...
            resp = requests.get(
                ...
                headers={
                    ...
                    "Referer": str(cfg.get("referer") or "https://www.tpex.org.tw/openapi/"),
                },
            )
            resp.raise_for_status()
            text_body = resp.text or ""
            if len(text_body.encode("utf-8", errors="ignore")) < 128:
                raise ValueError("下載內容過小，疑似失敗或非有效 JSON")

            # 嚴格 JSON：不可把 JSON 解析失敗的內容改用 CSV/cp950 硬讀。
            try:
                data = resp.json()
            except Exception as json_exc:
                raise RuntimeError(f"OpenAPI 回傳內容不是有效 JSON：{json_exc}")
            if isinstance(data, dict):
                data = data.get("data") or data.get("records") or data.get("result") or []
            if not isinstance(data, list):
...
def download_classification_book(force_refresh: bool = False, log_cb=None) -> Optional[Path]:
    ...
    if downloaded:
        meta = {
            ...
            "source": "LOCAL-CSV-OR-OPENAPI-JSON",
...
def _classification_csv_candidates_by_market(market: str = "ALL") -> list[Path]:
    ...
    3) cache_path（OpenAPI JSON fallback 成功後轉存的 CSV）

    不再優先讀取 RUNTIME_DIR 根目錄或舊 EXTERNAL_DATA_DIR 根目錄，避免過期檔案干擾。
    """
...
def get_prebreakout_report_path(date_str: str | None = None) -> Path:
    """固定輸出：reports/prebreakout_premarket_YYYYMMDD.xlsx。"""
...
class DBManager:
    ...
    def _init_external_decision_schema(self, cur):
        ...
        """)
        # PREBREAKOUT_UI_REPORT_PATCH：爆發前交易系統歷史資料 / Feature Cache / Signal 資料表
        cur.execute("""
        ...
        # R5E_ANNUAL_EPS_INDEX05_IMPORT_FIX_20260606：年度EPS為正式Gate核心表，必須在DB基礎schema建立，不能只依賴EPSForecastEngine.ensure_schema。
        for _col, _ddl in [
        ...
        CREATE TABLE IF NOT EXISTS external_revenue_history (
            stock_id TEXT, revenue_month TEXT, revenue REAL, mom REAL, yoy REAL, cumulative_revenue REAL, cumulative_yoy REAL,
            source_url TEXT, source_date TEXT, update_time TEXT,
            PRIMARY KEY(stock_id, revenue_month)
        )
        """)
        ...
        CREATE TABLE IF NOT EXISTS external_margin_history (
            stock_id TEXT, trade_date TEXT, margin_balance REAL, short_balance REAL, margin_change REAL, short_change REAL,
            margin_utilization REAL, retail_heat_score REAL, source_url TEXT, source_date TEXT, update_time TEXT,
            PRIMARY KEY(stock_id, trade_date)
        )
        """)
        cur.execute("""
        ...
        CREATE TABLE IF NOT EXISTS financial_quality_status (
            stock_id TEXT, quality_date TEXT, price_history_days INTEGER DEFAULT 0, eps_quarter_count INTEGER DEFAULT 0,
            revenue_month_count INTEGER DEFAULT 0, institutional_day_count INTEGER DEFAULT 0,
            data_quality_flag TEXT, data_gap_reason TEXT, update_time TEXT,
        ...
        CREATE TABLE IF NOT EXISTS trade_plan_prebreakout (
            stock_id TEXT, plan_date TEXT, stock_name TEXT, decision TEXT, entry_price REAL, stop_loss REAL, target1 REAL, target2 REAL, rr REAL,
            position_note TEXT, risk_note TEXT, source_signal_date TEXT, update_time TEXT,
        ...
        for sql in [
            ...
            "CREATE INDEX IF NOT EXISTS idx_prebreakout_signal_date ON prebreakout_signal_daily(signal_date, decision, prebreakout_total_score)",
    ...
    def schema_check_df(self) -> pd.DataFrame:
        required = ["stocks_master", "price_history", "ranking_result", "system_run_log", "external_data_log", "external_source_status", "market_snapshot", "external_revenue", "external_valuation", "financial_feature_daily", "external_dividend", "external_institutional", "external_margin", "external_event", "macro_module_score", "trade_plan", "quarterly_financial_history", "annual_eps_history", "external_revenue_history", "external_institutional_history", "external_margin_history", "technical_feature_daily", "financial_quality_status", "prebreakout_signal_daily", "trade_plan_prebreakout"]
        rows = []
        with self.lock:
...
def normalize_fiscal_quarter(value) -> tuple[int | None, int | None, str, str]:
    """統一 114/4、2025Q4、2025-Q4、2025Q04 格式，避免SQL漏抓。"""
    s = str(value or "").strip().upper().replace(" ", "")
    if not s or s in ("NAN", "NONE", "<NA>"):
        return None, None, "", ""
    m = re.match(r"^(\d{2,3})/([1-4])$", s)
    if m:
        fy = int(m.group(1)); q = int(m.group(2)); western = fy + 1911
        return fy, q, f"{fy}/{q}", f"{western}Q{q}"
    m = re.match(r"^(\d{4})[-/]?Q0?([1-4])$", s) or re.match(r"^(\d{4})[-/]?([1-4])$", s)
    if m:
        western = int(m.group(1)); q = int(m.group(2)); fy = western - 1911
        return fy, q, f"{fy}/{q}", f"{western}Q{q}"
    return None, None, "", ""
...
class PreBreakoutIntegratedEngine:
    ...
    def _latest_feature_date(self) -> str:
        try:
            with self.db.lock:
                row = self.db.conn.cursor().execute("SELECT MAX(date) FROM price_history").fetchone()
            if row and row[0]:
                return str(row[0])
        except Exception:
            pass
    ...
    def _copy_snapshot_to_history_if_needed(self):
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with self.db.lock:
            cur = self.db.conn.cursor()
            try:
                cnt = cur.execute("SELECT COUNT(*) FROM external_revenue_history").fetchone()[0]
                if int(cnt or 0) == 0:
                    cur.execute("""
                        INSERT OR IGNORE INTO external_revenue_history(stock_id,revenue_month,revenue,mom,yoy,cumulative_revenue,cumulative_yoy,source_date,update_time)
            ...
            try:
                ...
                if int(cnt or 0) == 0:
                    cur.execute("""
                        INSERT OR IGNORE INTO external_institutional_history(stock_id,trade_date,foreign_buy_sell,trust_buy_sell,dealer_buy_sell,total_inst,source_date,update_time)
                        SELECT stock_id,trade_date,foreign_buy_sell,trust_buy_sell,dealer_buy_sell,
                               COALESCE(foreign_buy_sell,0)+COALESCE(trust_buy_sell,0)+COALESCE(dealer_buy_sell,0),source_date,?
            ...
            try:
                ...
                if int(cnt or 0) == 0:
                    cur.execute("""
                        INSERT OR IGNORE INTO external_margin_history(stock_id,trade_date,margin_balance,short_balance,margin_change,short_change,margin_utilization,retail_heat_score,source_date,update_time)
                        SELECT stock_id,trade_date,margin_balance,short_balance,margin_change,short_change,margin_utilization,retail_heat_score,source_date,?
    ...
    def build_technical_feature_daily(self, feature_date: str | None = None, log_cb=None) -> int:
        ...
        with self.db.lock:
            price = pd.read_sql_query("SELECT stock_id,date,open,high,low,close,volume,turnover FROM price_history ORDER BY stock_id,date", self.db.conn)
...
def run_prebreakout_after_market_builder(db: DBManager | None = None, log_cb=None, progress_cb=None) -> dict:
    ...
    try:
        return PreBreakoutIntegratedEngine(db).run_after_market_builder(log_cb=log_cb)
    finally:
        if close_after: db.close()


def run_prebreakout_premarket_analyzer(db: DBManager | None = None, output_dir: Path | None = None, log_cb=None) -> Path:
    close_after=False
    if db is None:
        db=DBManager(DB_PATH); db.init_db(); close_after=True
    try:
        return PreBreakoutIntegratedEngine(db).export_premarket_excel(output_dir=output_dir or PREBREAKOUT_REPORT_DIR, log_cb=log_cb)
    finally:
        if close_after: db.close()
...
HIGH_GROWTH_COLUMN_MAP = {
    ...
    "stock_name": {"zh": "名稱", "en": "Stock Name"},
    "market": {"zh": "市場", "en": "Market"},
    "industry": {"zh": "產業", "en": "Industry"},
    "theme": {"zh": "題材", "en": "Theme"},
    "追蹤季度": {"zh": "追蹤季度", "en": "Tracking Quarter"},
    "基本面狀態": {"zh": "基本面狀態", "en": "Fundamental Status"},
    "投資分層": {"zh": "投資分層", "en": "Investment Tier"},
    "actual_q1_eps": {"zh": "2026Q1實際EPS", "en": "Actual Q1 EPS"},
    "predicted_q1_eps_model": {"zh": "模型預測Q1 EPS", "en": "Model Predicted Q1 EPS"},
    "predicted_q2_eps": {"zh": "預測Q2 EPS", "en": "Predicted Q2 EPS"},
    "predicted_q3_eps": {"zh": "預測Q3 EPS", "en": "Predicted Q3 EPS"},
    "eps_acceleration": {"zh": "EPS加速度", "en": "EPS Acceleration"},
    "eps_acceleration_pct": {"zh": "EPS加速度%", "en": "EPS Acceleration %"},
    "revenue_acceleration": {"zh": "營收加速度", "en": "Revenue Acceleration"},
    "prediction_error": {"zh": "預測誤差", "en": "Prediction Error"},
    "eps_surprise": {"zh": "黑馬指標", "en": "EPS Surprise"},
    "eps_surprise_pct": {"zh": "黑馬指標%", "en": "EPS Surprise %"},
    "pe_ratio": {"zh": "本益比", "en": "PE Ratio"},
    "peg_ratio": {"zh": "PEG", "en": "PEG Ratio"},
    "pe_score": {"zh": "PE分數", "en": "PE Score"},
    "peg_score": {"zh": "PEG分數", "en": "PEG Score"},
    "valuation_score_v412": {"zh": "估值分數", "en": "Valuation Score"},
    "v412_total_score": {"zh": "V4.12總分", "en": "V4.12 Total Score"},
    "v412_prediction_quality": {"zh": "預測品質", "en": "Prediction Quality"},
    "v412_leading_candidate": {"zh": "領先預測候選", "en": "Leading Forecast Candidate"},
    "v412_model_definition": {"zh": "V4.12模型定義", "en": "V4.12 Model Definition"},
    "base_annual_eps": {"zh": "2025全年EPS", "en": "Base Annual EPS"},
    "base_quarter_eps_for_prediction": {"zh": "預測用單季EPS基準", "en": "Quarter EPS Basis"},
    "prediction_basis": {"zh": "預測基準", "en": "Prediction Basis"},
    ...
    "q1_vs_annual_ratio": {"zh": "Q1/年EPS", "en": "Q1/Annual EPS"},
    "q1_vs_annual_ratio_raw": {"zh": "Q1/年EPS(原始)", "en": "Raw Q1/Annual EPS"},
    "q1_ratio_quality": {"zh": "Q1倍率品質", "en": "Q1 Ratio Quality"},
    "forecast_vs_annual_ratio": {"zh": "預估/年EPS", "en": "Forecast/Annual EPS"},
    "ttm_vs_annual_ratio": {"zh": "TTM/年EPS", "en": "TTM/Annual EPS"},
    "eps_source_used": {"zh": "EPS來源", "en": "EPS Source"},
    "avg_revenue_yoy": {"zh": "平均YoY", "en": "Average YoY"},
    "avg_revenue_yoy_used": {"zh": "採用平均YoY", "en": "Used Average YoY"},
    "revenue_month_count_hit": {"zh": "達標營收月數", "en": "Revenue Hit Months"},
    "revenue_month_count_required": {"zh": "要求營收月數", "en": "Required Revenue Months"},
    "revenue_stair_gate": {"zh": "營收階梯", "en": "Revenue Stair Gate"},
    "close": {"zh": "現價", "en": "Close Price"},
    "current_price": {"zh": "目前價格", "en": "Current Price"},
    "rsi14": {"zh": "RSI", "en": "RSI"},
    "trend_score": {"zh": "趨勢分", "en": "Trend Score"},
    "風險旗標": {"zh": "風險旗標", "en": "Risk Flag"},
    "建議動作": {"zh": "建議動作", "en": "Suggested Action"},
    "資料缺口/未通過原因": {"zh": "資料缺口/未通過原因", "en": "Data Gap / Fail Reason"},
    "規則版本": {"zh": "規則版本", "en": "Rule Version"},
}

# V4.15：補齊高成長EPS報表欄位名稱，不讓ZH報表殘留英文內部變數，也不讓EN報表殘留中文表頭。
HIGH_GROWTH_COLUMN_MAP.update({
    "項目": {"zh": "項目", "en": "Item"},
    "內容": {"zh": "內容", "en": "Content"},
    "資料表": {"zh": "資料表", "en": "Table"},
    "是否存在": {"zh": "是否存在", "en": "Exists"},
    "筆數": {"zh": "筆數", "en": "Rows"},
    "主要欄位": {"zh": "主要欄位", "en": "Main Columns"},
    "V4用途": {"zh": "V4用途", "en": "V4 Purpose"},
    "判定": {"zh": "判定", "en": "Judgement"},
    "狀態": {"zh": "狀態", "en": "Status"},
    "說明": {"zh": "說明", "en": "Description"},
    "序號": {"zh": "序號", "en": "No."},
    "模組": {"zh": "模組", "en": "Module"},
    "新增/修改": {"zh": "新增/修改", "en": "Add/Modify"},
    "程式位置": {"zh": "程式位置", "en": "Program Location"},
    "驗收點": {"zh": "驗收點", "en": "Validation Point"},
    "actual_eps_cumulative": {"zh": "實際累計EPS", "en": "Actual Cumulative EPS"},
    "selected_actual_eps_cumulative": {"zh": "所選季度實際累計EPS", "en": "Selected Actual Cumulative EPS"},
    "current_eps_ttm": {"zh": "近四季EPS", "en": "Current EPS TTM"},
    "selected_actual_vs_annual_ratio": {"zh": "所選實際/年EPS", "en": "Selected Actual/Annual EPS"},
    "selected_actual_vs_annual_ratio_raw": {"zh": "所選實際/年EPS(原始)", "en": "Raw Selected Actual/Annual EPS"},
    "forecast_vs_annual_ratio_raw": {"zh": "預估/年EPS(原始)", "en": "Raw Forecast/Annual EPS"},
    "eps_ratio": {"zh": "EPS倍率", "en": "EPS Ratio"},
    "eps_ratio_legacy": {"zh": "舊EPS倍率", "en": "Legacy EPS Ratio"},
    "eps_ratio_definition": {"zh": "EPS倍率定義", "en": "EPS Ratio Definition"},
    "forecast_yoy_cap_flag": {"zh": "營收YoY封頂旗標", "en": "Revenue YoY Cap Flag"},
    "forecast_risk_flag": {"zh": "預估風險旗標", "en": "Forecast Risk Flag"},
    "predicted_q2_eps_score": {"zh": "預測Q2 EPS分數", "en": "Predicted Q2 EPS Score"},
    "eps_acceleration_score_v412": {"zh": "EPS加速度分數", "en": "EPS Acceleration Score"},
    "revenue_acceleration_score_v412": {"zh": "營收加速度分數", "en": "Revenue Acceleration Score"},
    "eps_ttm_source": {"zh": "近四季EPS來源", "en": "EPS TTM Source"},
    "eps_ttm_quality": {"zh": "近四季EPS品質", "en": "EPS TTM Quality"},
    "eps_ttm_source_date": {"zh": "近四季EPS來源日期", "en": "EPS TTM Source Date"},
    "eps_ttm_fiscal_year_quarter": {"zh": "近四季EPS財報季", "en": "EPS TTM Fiscal Quarter"},
    "base_eps_source": {"zh": "年度EPS來源", "en": "Base EPS Source"},
    "eps_data_quality": {"zh": "EPS資料品質", "en": "EPS Data Quality"},
    "base_eps_quality": {"zh": "年度EPS品質", "en": "Base EPS Quality"},
    "avg_revenue_yoy_raw": {"zh": "平均營收YoY(原始)", "en": "Raw Average Revenue YoY"},
    "min_revenue_yoy": {"zh": "最低營收YoY", "en": "Minimum Revenue YoY"},
    "revenue_complete_gate": {"zh": "營收完整Gate", "en": "Revenue Complete Gate"},
    "revenue_yoy_gate": {"zh": "營收YoY Gate", "en": "Revenue YoY Gate"},
    "revenue_partial_yoy_gate": {"zh": "營收部分YoY Gate", "en": "Partial Revenue YoY Gate"},
    "revenue_strict_gate": {"zh": "營收嚴格Gate", "en": "Strict Revenue Gate"},
    "strict_pass": {"zh": "正式嚴格通過", "en": "Strict Actual Pass"},
    "forecast_candidate": {"zh": "預測候選", "en": "Forecast Candidate"},
    "ttm_proxy_candidate": {"zh": "近四季EPS代理候選", "en": "EPS TTM Proxy Candidate"},
    "proxy_pass": {"zh": "代理通過", "en": "Proxy Pass"},
    "macd_hist": {"zh": "MACD柱狀", "en": "MACD Histogram"},
    "volume_ratio": {"zh": "量比", "en": "Volume Ratio"},
})


# V4.15-R2：報表資料品質與驗證語意補強欄位。
HIGH_GROWTH_COLUMN_MAP.update({
    "official_eps_only_pass": {"zh": "正式EPS本身通過", "en": "Official EPS Only Pass"},
    "official_eps_revenue_pass": {"zh": "正式EPS+營收雙驗證通過", "en": "Official EPS + Revenue Pass"},
    "legacy_strict_pass": {"zh": "Legacy嚴格通過", "en": "Legacy Strict Pass"},
    "effective_valuation_score": {"zh": "有效估值分數", "en": "Effective Valuation Score"},
    "effective_valuation_weight": {"zh": "有效估值權重", "en": "Effective Valuation Weight"},
    "outlier_penalty": {"zh": "極端值扣分", "en": "Outlier Penalty"},
    "outlier_risk_flag": {"zh": "極端值風險旗標", "en": "Outlier Risk Flag"},
    "required_month": {"zh": "要求月份", "en": "Required Month"},
    "available_rows": {"zh": "可用筆數", "en": "Available Rows"},
    "available_stock_count": {"zh": "可用股票數", "en": "Available Stock Count"},
    "coverage_status": {"zh": "覆蓋狀態", "en": "Coverage Status"},
    "missing_months": {"zh": "缺失月份", "en": "Missing Months"},
    "run_evidence_item": {"zh": "執行證據項目", "en": "Run Evidence Item"},
    "run_evidence_value": {"zh": "執行證據內容", "en": "Run Evidence Value"},
    "validation_metric": {"zh": "驗證指標", "en": "Validation Metric"},
    "validation_value": {"zh": "驗證數值", "en": "Validation Value"},
    "validation_note": {"zh": "驗證說明", "en": "Validation Note"},
})
...
HIGH_GROWTH_SHEET_MAP = {
    ...
    "11_Run證據": {"zh": "11_Run證據", "en": "11_Run_Evidence"},
...
def high_growth_sheet_label(name: str, language: str | None = None) -> str:
    ...
    item = HIGH_GROWTH_SHEET_MAP.get(str(name))
    if not item:
        return str(name)
    return item.get("en" if lang == "EN" else "zh", str(name))

def high_growth_column_label(key: str, language: str | None = None) -> str:
    lang = (language or REPORT_LANGUAGE or "ZH").strip().upper()
    key_str = str(key)
    if key_str.startswith("rev_"):
        ym = key_str.replace("rev_", "")
        return f"營收{ym}" if lang == "ZH" else (f"Revenue {ym}" if lang == "EN" else f"營收{ym}\n(Revenue {ym})")
    if key_str.startswith("yoy_"):
        ym = key_str.replace("yoy_", "")
        return f"營收年增率{ym}" if lang == "ZH" else (f"Revenue YoY {ym}" if lang == "EN" else f"營收年增率{ym}\n(Revenue YoY {ym})")
    item = HIGH_GROWTH_COLUMN_MAP.get(key_str)
    if not item:
        return key_str
    zh = item.get("zh", str(key))
    en = item.get("en", str(key))
    if lang == "ZH":
        return zh
    if lang == "EN":
        return en
...
def localize_high_growth_dataframe_columns(df: pd.DataFrame, language: str | None = None) -> pd.DataFrame:
    if df is None or not isinstance(df, pd.DataFrame):
        return df
    out = df.copy()
    out.columns = [high_growth_column_label(c, language) for c in out.columns]
    return out
...
HIGH_GROWTH_QUARTER_REVENUE_MONTHS = {
    "Q1": ["202602", "202603", "202604"],
    "Q2": ["202605", "202606", "202607"],
    "Q3": ["202608", "202609", "202610"],
    "Q4": ["202611", "202612", "202701"],
}
...
def normalize_high_growth_quarters(quarters=None) -> list[str]:
    ...
    if isinstance(quarters, str):
        raw = re.split(r"[,/|;\s]+", quarters.strip().upper())
    else:
        raw = [str(q).strip().upper() for q in quarters]
    out = []
    for q in raw:
        if not q:
            continue
        q = q.replace("季度", "").replace("第", "").replace("季", "")
        if q in ("1", "一"):
            q = "Q1"
        elif q in ("2", "二"):
            q = "Q2"
            ...
            q = "Q4"
        if q in HIGH_GROWTH_QUARTER_REVENUE_MONTHS and q not in out:
            out.append(q)
    return out or list(HIGH_GROWTH_DEFAULT_QUARTERS)


class RevenueAccelerationEngine:
    """月營收加速度引擎：以所選季度對應月份檢查 YoY 暴增與階梯式月月走高。"""
    ...
    def _read_sql(self, sql: str, params=None) -> pd.DataFrame:
        try:
            ...
            log_warning(f"[RevenueAccelerationEngine] SQL讀取失敗：{exc}｜SQL={sql[:120]}")
            return pd.DataFrame()
    ...
    def normalize_month(value) -> str:
        ...
        R3 修正重點：
        - 支援 OpenAPI / DB / Excel 可能出現的 115年04月、115/04、115-04、11504、202604、2026年04月。
        - 舊版只移除 - /，遇到「115年04月」會無法命中，導致 external_revenue 有資料但 required_months 全部 MISSING。
        - 失敗時回傳空字串，不用目前月份硬補，避免偽造覆蓋率。
        """
        if value is None:
            return ""
        ...
        # 西元：2026年04月 / 2026-04 / 2026/4 / 202604
        m = re.search(r"(20\d{2}|19\d{2})\D{0,3}(\d{1,2})", s)
        if m:
            y = int(m.group(1)); mm = int(m.group(2))
            if 1 <= mm <= 12:
                return f"{y:04d}{mm:02d}"

        # 民國：115年04月 / 115/04 / 115-4 / 民國115年4月 / 11504
        m = re.search(r"(\d{2,3})\D{0,3}(\d{1,2})", s)
        if m:
            y = int(m.group(1)); mm = int(m.group(2))
            if y < 1911 and 1 <= mm <= 12:
                return f"{y + 1911:04d}{mm:02d}"

        digits = re.sub(r"\D", "", s)
        if len(digits) >= 6:
            # 202604
            y = int(digits[:4]); mm = int(digits[4:6])
            if y >= 1911 and 1 <= mm <= 12:
                return f"{y:04d}{mm:02d}"
        if len(digits) == 5:
            # 11504
            y = int(digits[:3]); mm = int(digits[3:5])
            if 1 <= mm <= 12:
                return f"{y + 1911:04d}{mm:02d}"
        return ""

    def required_months(self, quarters=None) -> list[str]:
        months = []
        for q in normalize_high_growth_quarters(quarters):
            for m in HIGH_GROWTH_QUARTER_REVENUE_MONTHS.get(q, []):
                if m not in months:
                    months.append(m)
        return months
    ...
    def load_monthly_revenue(self, quarters=None) -> pd.DataFrame:
        ...
        for table, priority in [("external_revenue_history", 1), ("external_revenue", 2)]:
            if not self._table_exists(table):
                continue
            df = self._read_sql(f"SELECT stock_id, revenue_month, revenue, mom, yoy, cumulative_revenue, cumulative_yoy, source_date, update_time FROM {table}")
            if df is None or df.empty:
                continue
            df = df.copy()
            df["stock_id"] = df["stock_id"].map(normalize_stock_id)
            df["revenue_month_norm"] = df["revenue_month"].map(self.normalize_month)
            df["revenue"] = pd.to_numeric(df.get("revenue"), errors="coerce")
            df["yoy"] = pd.to_numeric(df.get("yoy"), errors="coerce")
            df["mom"] = pd.to_numeric(df.get("mom"), errors="coerce")
            df["source_table"] = table
            df["_priority"] = priority
            df = df[df["revenue_month_norm"].isin(months)].copy()
            parts.append(df)
        if not parts:
            return pd.DataFrame(columns=["stock_id", "revenue_month_norm", "revenue", "yoy", "mom", "source_table"])
        x = pd.concat(parts, ignore_index=True)
        x = x.sort_values(["stock_id", "revenue_month_norm", "_priority"], ascending=[True, True, True])
    ...
    def build_revenue_features(self, master: pd.DataFrame, quarters=None) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if base.empty:
            return base, rev
        for m in months:
            mdf = rev[rev["revenue_month_norm"].eq(m)][["stock_id", "revenue", "yoy", "mom", "source_table"]].copy() if not rev.empty else pd.DataFrame(columns=["stock_id", "revenue", "yoy", "mom", "source_table"])
            mdf = mdf.rename(columns={"revenue": f"rev_{m}", "yoy": f"yoy_{m}", "mom": f"mom_{m}", "source_table": f"rev_source_{m}"})
            base = base.merge(mdf, on="stock_id", how="left")
        yoy_cols = [f"yoy_{m}" for m in months]
        rev_cols = [f"rev_{m}" for m in months]
        for c in yoy_cols + rev_cols:
            if c not in base.columns:
                base[c] = np.nan
            base[c] = pd.to_numeric(base[c], errors="coerce")
        base["selected_quarters"] = "+".join(quarters)
...
class RevenueOpenAPICacheDownloader:
    ...
    定位：
    - 不取代 R4 原本 fetch_revenue / writer / history fallback。
    - 僅在原流程缺月份或官方抓取失敗時，補讀/補抓官方 OpenAPI 快取。
    ...
    @staticmethod
    def _find_revenue_columns(raw: pd.DataFrame) -> dict:
        col_map = {}
    ...
    def _read_remote_json_df(self, url: str, referer: str = "") -> tuple[pd.DataFrame, str]:
        resp = requests.get(
            ...
            headers={
                "User-Agent": "Mozilla/5.0",
                "Accept": "application/json,text/plain,*/*",
                "Referer": referer or ("https://www.tpex.org.tw/openapi/" if "tpex.org.tw" in str(url) else "https://openapi.twse.com.tw/"),
            },
        )
        ...
        try:
            ...
            raise RuntimeError(f"OpenAPI cache JSON解析失敗：{exc}") from exc
        if isinstance(payload, dict):
            payload = payload.get("data") or payload.get("records") or payload.get("result") or []
        if not isinstance(payload, list):
            raise RuntimeError("OpenAPI cache JSON格式不是陣列")
        return pd.DataFrame(payload).astype(str).fillna(""), status

    def parse_revenue_openapi(self, raw: pd.DataFrame, source_label: str = "") -> tuple[pd.DataFrame, str]:
        if raw is None or raw.empty:
            return pd.DataFrame(), f"{source_label} cache raw rows=0"
        col_map = self._find_revenue_columns(raw)
        if "stock_id" not in col_map or "revenue" not in col_map or "revenue_month" not in col_map:
            return pd.DataFrame(), f"{source_label} cache欄位無法辨識：{list(raw.columns)[:20]}"
        rows = []
        for _, r in raw.iterrows():
            ...
            revenue = self._num(r.get(col_map.get("revenue"), np.nan), default=np.nan)
            if not sid or not month or (not np.isfinite(revenue)) or float(revenue) <= 0:
                continue
            rows.append({
                "stock_id": sid,
                "revenue_month": month,
                "revenue": float(revenue),
                "mom": self._num(r.get(col_map.get("mom"), np.nan), default=np.nan),
                "yoy": self._num(r.get(col_map.get("yoy"), np.nan), default=np.nan),
                "cumulative_revenue": self._num(r.get(col_map.get("cumulative_revenue"), np.nan), default=np.nan),
                "cumulative_yoy": self._num(r.get(col_map.get("cumulative_yoy"), np.nan), default=np.nan),
    ...
    def latest_cache_file(self) -> Path | None:
        files = sorted(self.cache_dir.glob("revenue_openapi_*.csv"), key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True)
        return files[0] if files else None

    def read_latest_cache(self) -> pd.DataFrame:
        p = self.latest_cache_file()
        if p is None:
            return pd.DataFrame()
        last_error = None
        for enc in ("utf-8-sig", "utf-8", "big5", "cp950"):
            try:
                ...
                return df[["stock_id", "revenue_month", "revenue", "mom", "yoy", "cumulative_revenue", "cumulative_yoy", "source_date", "source_url", "update_time"]].drop_duplicates(["stock_id", "revenue_month"], keep="last")
            except Exception as exc:
                last_error = exc
                continue
    ...
    def download_latest_cache(self, include_optional: bool = False) -> tuple[pd.DataFrame, Path | None, str]:
        ...
        if include_optional:
            endpoints.update(self.optional_endpoints)
        for label, url in endpoints.items():
            try:
                raw, status = self._read_remote_json_df(url)
                parsed, err = self.parse_revenue_openapi(raw, source_label=label)
                if parsed is None or parsed.empty:
                    errors.append(err or f"{label} rows=0")
                    continue
                parsed["source_url"] = url
                parts.append(parsed)
                log_info(f"[REVENUE][OPENAPI_CACHE][{label}] download rows={len(parsed)} status={status}")
            except Exception as exc:
    ...
    def supplement_missing_only(self, existing_df: pd.DataFrame | None, required_months: list[str], force_download: bool = False) -> pd.DataFrame:
        ...
        if not existing.empty and "revenue_month" in existing.columns:
            existing["stock_id"] = existing.get("stock_id", "").map(normalize_stock_id)
            existing["revenue_month"] = existing["revenue_month"].map(RevenueAccelerationEngine.normalize_month)
        ...
        if not existing.empty and {"stock_id", "revenue_month"}.issubset(existing.columns):
            exist_keys = set(zip(existing["stock_id"].astype(str), existing["revenue_month"].astype(str)))
            cache_df = cache_df[~cache_df.apply(lambda r: (str(r.get("stock_id", "")), str(r.get("revenue_month", ""))) in exist_keys, axis=1)].copy()
        cache_df = cache_df[pd.to_numeric(cache_df.get("revenue"), errors="coerce") > 0].copy()
        if cache_df.empty:
            return pd.DataFrame()
...
class EPSForecastEngine:
    ...
    def __init__(self, db: DBManager, base_year: int = HIGH_GROWTH_BASE_YEAR, track_year: int = HIGH_GROWTH_TRACK_YEAR):
        ...
        self.track_year = int(track_year)
        self.ensure_schema()
    ...
    def _read_sql(self, sql: str, params=None) -> pd.DataFrame:
        try:
            ...
            log_warning(f"[EPSForecastEngine] SQL讀取失敗：{exc}｜SQL={sql[:120]}")
            return pd.DataFrame()
    ...
    @staticmethod
    def normalize_quarter_token(value) -> str:
        s = str(value or "").strip().upper().replace(" ", "")
    ...
    def ensure_required_official_eps_from_mops(self, quarters=None, force: bool = False) -> dict:
        ...
        quarters = normalize_high_growth_quarters(quarters)
        results = {}
        results["base_annual_local_114q4"] = self.ensure_annual_eps_from_local_114q4_file(self.base_year, force=force)
        if str(results["base_annual_local_114q4"].get("status", "")).lower() in ("exists", "imported"):
            results["base_annual"] = results["base_annual_local_114q4"]
        else:
    ...
    def _annual_eps_local_candidate_paths(self, fiscal_year: int | None = None, quarter: str = "Q4") -> list[Path]:
        ...
        嚴格限制：若檔案內容有年度/季別欄位，必須為 fiscal_year=2025 / quarter=4；若沒有
        年度/季別欄位，檔名必須含 114Q4 或 2025Q4，避免把 115Q1 誤寫成 2025 全年EPS。
        """
        fiscal_year = int(fiscal_year or self.base_year)
        ...
        for p in [
            ...
            RUNTIME_DIR / "data" / "annual_eps",
            RUNTIME_DIR / "data" / "twse_c05001",
            RUNTIME_DIR / "data",
            RUNTIME_DIR,
            BASE_DIR / "data" / "eps",
            BASE_DIR / "data" / "annual_eps",
            BASE_DIR / "data" / "twse_c05001",
            BASE_DIR / "data",
            BASE_DIR,
        ]:
            _add(p)
        ...
        patterns = [
            ...
            f"*{roc_year}*Q4*EPS*.csv", f"*{roc_year}*Q4*EPS*.json",
    ...
    def _parse_annual_eps_local_table(self, path: Path, fiscal_year: int | None = None, quarter: int = 4) -> pd.DataFrame:
        ...
        if ext == ".json":
            ...
            return pd.DataFrame()
        if raw is None or raw.empty:
            log_warning(f"[HIGH_GROWTH][ANNUAL_EPS_LOCAL] raw empty file={p}")
            return pd.DataFrame()
        raw = raw.copy().fillna("")
        def _find_col(candidates):
            ...
            for cand in candidates:
                ...
                if key in norm_map:
                    return norm_map[key]
            return None
        code_col = _find_col(["公司代號", "公司代碼", "股票代號", "證券代號", "Code", "SecuritiesCompanyCode", "stock_id"])
        name_col = _find_col(["公司名稱", "公司簡稱", "證券名稱", "股票名稱", "Name", "CompanyName", "stock_name"])
        year_col = _find_col(["年度", "財報年度", "會計年度", "Year", "fiscal_year", "fiscal_year_roc"])
        quarter_col = _find_col(["季別", "財報季別", "季度", "Quarter", "Season", "quarter"])
        eps_col = _find_col(["基本每股盈餘（元）", "基本每股盈餘(元)", "基本每股盈餘", "EarningsPerShare", "BasicEarningsPerShare", "EPS", "eps", "eps_full_year"])
        if code_col is None or eps_col is None:
            raise RuntimeError(f"本機年度EPS檔欄位不足 file={p} code_col={code_col} eps_col={eps_col} columns={list(raw.columns)[:40]}")
        filename_token_ok = bool(re.search(rf"({roc_year}|{fiscal_year}).*Q?4", p.name, flags=re.I))
        if year_col is not None and quarter_col is not None:
            year_token = raw[year_col].astype(str).str.extract(r"(\d{2,4})", expand=False)
            year_num = pd.to_numeric(year_token, errors="coerce")
            year_western = year_num.where(year_num >= 1000, year_num + 1911)
            quarter_token = raw[quarter_col].astype(str).str.upper().str.extract(r"(?:Q|第)?0?([1-4])", expand=False)
            q_num = pd.to_numeric(quarter_token, errors="coerce")
            mask = year_western.eq(float(fiscal_year)) & q_num.eq(float(q))
            log_info(f"[HIGH_GROWTH][ANNUAL_EPS_LOCAL] file={p} target={fiscal_year}Q{q} matched={int(mask.sum())} rejected={int((~mask).sum())} year_counts={year_token.fillna('').astype(str).value_counts().head(5).to_dict()} quarter_counts={quarter_token.fillna('').astype(str).value_counts().head(5).to_dict()}")
            raw = raw.loc[mask].copy()
        elif not filename_token_ok:
            raise RuntimeError(f"本機年度EPS檔缺年度/季別欄位，且檔名未標示114Q4/2025Q4，禁止寫入 annual_eps_history：{p.name}")
        out = pd.DataFrame({
            "stock_id": raw[code_col].astype(str).str.extract(r"(\d{4,5})", expand=False).fillna("").map(normalize_stock_id),
            "stock_name": raw[name_col].astype(str).str.strip() if name_col in raw.columns else "",
        ...
        if out.empty:
            log_warning(f"[HIGH_GROWTH][ANNUAL_EPS_LOCAL] parsed empty after EPS clean file={p}")
            return pd.DataFrame()
        out["fiscal_year"] = fiscal_year
        out["fiscal_year_roc"] = roc_year
        out["quarter"] = q
        out["eps_cumulative"] = out["eps_full_year"]
        out["eps_quarter"] = np.nan
        out["eps_prev_cumulative"] = np.nan
        out["eps_calc_method"] = "LOCAL_114Q4_Q4_CUMULATIVE_AS_ANNUAL_EPS"
        out["source_type"] = "LOCAL_114Q4_EPS_FILE"
        out["source_url"] = "LOCAL_FILE"
        out["source_file"] = str(p)
        out["source_date"] = f"{fiscal_year}Q{q}"
        out["update_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        out = out.drop_duplicates(subset=["stock_id", "fiscal_year"], keep="last")
        log_info(f"[HIGH_GROWTH][ANNUAL_EPS_LOCAL] parsed file={p} rows={len(out)} sample={out[['stock_id','eps_full_year']].head(3).to_dict('records')}")
        return out
    ...
    def _twse_c05001_candidate_paths(self, fiscal_year: int | None = None, quarter: str = "Q4") -> list[Path]:
        ...
        def _add_root(p):
            try:
                ...
                pass
        for p in [
            RUNTIME_DIR / "data" / "twse_c05001" / f"{fiscal_year}{q}",
            RUNTIME_DIR / "data" / "twse_c05001",
            RUNTIME_DIR / "data" / "eps",
            RUNTIME_DIR, RUNTIME_DIR / "data", RUNTIME_DIR / "reports",
            BASE_DIR / "data" / "twse_c05001" / f"{fiscal_year}{q}",
            BASE_DIR / "data" / "twse_c05001",
            BASE_DIR / "data" / "eps",
            BASE_DIR, BASE_DIR / "data", BASE_DIR / "reports",
        ]:
            _add_root(p)
        try:
        ...
        patterns = [
            ...
            f"{roc_year}{q}_C05001.zip", f"{roc_year}{q}.zip", f"*{roc_year}{q}*C05001*.zip", f"*C05001*{roc_year}*{q}*.zip",
            f"{roc_year}{q}.xlsx", f"{roc_year}{q}.xls", f"*{roc_year}{q}*C05001*.xlsx", f"*{roc_year}{q}*C05001*.xls",
        ]
        out, seen = [], set()
    ...
    def _parse_twse_c05001_eps_xlsx(self, xlsx_path: Path, fiscal_year: int | None = None) -> pd.DataFrame:
        ...
        if out.empty:
            ...
            return pd.DataFrame()
        out["fiscal_year"] = fiscal_year
        out["fiscal_year_roc"] = fiscal_year - 1911
        out["quarter"] = 4
        out["eps_cumulative"] = out["eps_full_year"]
        out["eps_quarter"] = np.nan
        out["eps_prev_cumulative"] = np.nan
        out["eps_calc_method"] = "TWSE_C05001_Q4_CUMULATIVE_AS_ANNUAL_EPS"
    ...
    def _write_annual_eps_history(self, df: pd.DataFrame, fiscal_year: int, source_label: str = "") -> int:
        ...
        write["fiscal_year_roc"] = int(fiscal_year) - 1911
        write["quarter"] = 4
        ...
        try:
            log_info(
                f"[HIGH_GROWTH][ANNUAL_EPS_HISTORY] written_rows={len(write)} fiscal_year={int(fiscal_year)} "
                f"sources={write['source_type'].astype(str).value_counts().to_dict()} sample={write[['stock_id','eps_full_year']].head(3).to_dict('records')}"
            )
        except Exception:
            pass
    ...
    def _download_twse_c05001_from_index(self, fiscal_year: int | None = None, quarter: str = "Q4") -> Path | None:
        ...
        index_urls = [
            ...
            f"https://www.twse.com.tw/zh/trading/statistics/index05.html?year={fiscal_year}",
            "https://www.twse.com.tw/zh/trading/statistics/index05.html",
            f"https://wwwc.twse.com.tw/en/trading/statistics/index05.html?year={fiscal_year}",
            "https://wwwc.twse.com.tw/en/trading/statistics/index05.html",
            f"https://wwwc.twse.com.tw/zh/trading/statistics/index05.html?year={fiscal_year}",
        ...
        def _add_candidate(u: str, reason: str = ""):
            ...
            if score_hit and u not in candidate_urls:
                candidate_urls.append(u)
                log_info(f"[HIGH_GROWTH][C05001][INDEX05_LINK] add reason={reason} url={u}")

        for base_url in index_urls:
            try:
                resp = requests.get(base_url, timeout=(10, 30), headers=headers)
                log_info(f"[HIGH_GROWTH][C05001][INDEX05_PAGE] url={base_url} status={resp.status_code} bytes={len(resp.content) if resp.content else 0}")
        ...
        static_names = [
            f"{fiscal_year}{q}_C05001.zip", f"{fiscal_year}{q}.zip", f"{fiscal_year}{q}.xls", f"{fiscal_year}{q}.xlsx",
    ...
    def load_base_annual_eps(self) -> pd.DataFrame:
        ...
        if self._table_exists("annual_eps_history"):
            ...
            try:
                with self.db.lock:
                    annual_cols = {str(r[1]) for r in self.db.conn.cursor().execute("PRAGMA table_info(annual_eps_history)").fetchall()}
            except Exception:
                annual_cols = set()
    ...
    def load_actual_eps_by_quarter(self, quarters=None) -> pd.DataFrame:
        ...
        token2 = df.get("fiscal_year_quarter", "").map(self.normalize_quarter_token) if "fiscal_year_quarter" in df.columns else pd.Series("", index=df.index)
        mask = (((fy == self.track_year) | (fy == self.track_year - 1911)) & qn.isin(q_numbers))
        for q in quarters:
            mask = mask | token1.eq(f"{self.track_year}{q}") | token2.eq(f"{self.track_year}{q}")
        df = df.loc[mask].copy()
        if df.empty:
            return pd.DataFrame(columns=["stock_id"])
        ...
        for q in quarters:
            if q not in pivot.columns:
                pivot[q] = np.nan
            pivot[f"eps_{self.track_year}_{q.lower()}"] = pivot[q]
        eps_cols = [f"eps_{self.track_year}_{q.lower()}" for q in quarters]
        pivot["actual_eps_cumulative"] = pivot[eps_cols].sum(axis=1, min_count=len(eps_cols))
        pivot["actual_eps_quarter_count_hit"] = pivot[eps_cols].notna().sum(axis=1)
    ...
    def _latest_rows(self, table: str, date_col: str, value_cols: list[str]) -> pd.DataFrame:
        if not self._table_exists(table):
            return pd.DataFrame(columns=["stock_id"])
        cols = ["stock_id", date_col] + [c for c in value_cols if c]
        cols_sql = ", ".join(cols)
        q = f"""
        SELECT t.{cols_sql}
        FROM {table} t
        JOIN (
            SELECT stock_id, MAX({date_col}) AS max_date
            FROM {table}
            GROUP BY stock_id
        ) m ON t.stock_id=m.stock_id AND t.{date_col}=m.max_date
        """
        return self._read_sql(q)

    def load_eps_ttm_proxy(self) -> pd.DataFrame:
        """讀取 DB 既有 EPS_TTM 代理來源。

        優先序：
        1. external_valuation.eps_ttm / eps（官方估值表反推，fiscal_year_quarter 多為 115/1）
        2. financial_feature_daily.eps_ttm（既有 EPS 矩陣特徵，涵蓋較廣）
        3. ranking_result.eps_ttm（排行榜中繼欄位）
        """
        parts = []
        if self._table_exists("external_valuation"):
            v = self._latest_rows("external_valuation", "data_date", ["eps_ttm", "eps", "pe", "close_price", "fiscal_year_quarter", "source_date"])
            if v is not None and not v.empty:
                v["current_eps_ttm"] = pd.to_numeric(v.get("eps_ttm"), errors="coerce").fillna(pd.to_numeric(v.get("eps"), errors="coerce"))
        ...
        if self._table_exists("financial_feature_daily"):
            ...
            if f is not None and not f.empty:
                f["current_eps_ttm"] = pd.to_numeric(f.get("eps_ttm"), errors="coerce")
        ...
        if self._table_exists("ranking_result"):
            ...
            if r is not None and not r.empty:
                r["current_eps_ttm"] = pd.to_numeric(r.get("eps_ttm"), errors="coerce")
    ...
    def build_eps_features(self, master: pd.DataFrame, revenue_features: pd.DataFrame, quarters=None) -> pd.DataFrame:
        ...
        # V4.9：EPS欄位透明化與Ratio定義修正
        # 1) Forecast EPS 必須與 Actual EPS 分欄，避免把預估值誤當實際Q1 EPS。
        # 2) eps_ratio 不再用 forecast_eps_cumulative / base_annual_eps；
        #    改為「所選正式季度實際EPS / 2025全年EPS」，Q1畫面即為 actual_q1_eps / base_annual_eps。
        # 3) Forecast 與 TTM 另外獨立顯示倍率，不可混用於嚴格通過條件。
        avg_yoy = pd.to_numeric(x.get("avg_revenue_yoy"), errors="coerce")
        x["avg_revenue_yoy_raw"] = avg_yoy
        x["avg_revenue_yoy_used"] = avg_yoy.clip(lower=-80, upper=300)
        x["forecast_yoy_cap_flag"] = np.select(
            [avg_yoy.gt(300), avg_yoy.lt(-80)],
            ["YOY_CAP_HIGH", "YOY_CAP_LOW"],
            default="YOY_NORMAL",
        )

        base_quarter_eps = x["base_annual_eps"] / 4.0 * q_count
        yoy_factor_raw = 1.0 + (avg_yoy.fillna(0) / 100.0) * 0.55
        yoy_factor_used = 1.0 + (x["avg_revenue_yoy_used"].fillna(0) / 100.0) * 0.55
        x["forecast_eps_cumulative_raw"] = (base_quarter_eps * yoy_factor_raw).round(4)
        x["forecast_eps_cumulative"] = (base_quarter_eps * yoy_factor_used).round(4)
        x.loc[x["base_annual_eps"].isna() | avg_yoy.isna(), ["forecast_eps_cumulative_raw", "forecast_eps_cumulative"]] = np.nan
        x["forecast_risk_flag"] = np.select(
            [
                x["base_annual_eps"].abs().lt(1),
                avg_yoy.gt(300),
                avg_yoy.lt(-80),
            ],
            [
        ...
        # V4.15-R2：YoY封頂/極端值須明確進入報表與排序風險欄，避免單月小基期污染黑馬池。
        x["outlier_risk_flag"] = np.select(
            [
                x["forecast_risk_flag"].isin(["YOY_OUTLIER_CAPPED", "YOY_NEGATIVE_CAPPED"]),
                x["forecast_yoy_cap_flag"].isin(["YOY_CAP_HIGH", "YOY_CAP_LOW"]),
            ],
            [
                x["forecast_risk_flag"],
                x["forecast_yoy_cap_flag"],
            ],
            default="OK",
        )
        ...
        forecast_available = x["forecast_eps_cumulative"].notna()
        ttm_available = x["current_eps_ttm"].notna() & x["current_eps_ttm"].gt(0)

        # actual_q1_eps：使用者指定的核心欄位，代表 2026Q1 正式實際EPS。
        q1_col = f"eps_{self.track_year}_q1"
        x["actual_q1_eps"] = pd.to_numeric(x.get(q1_col), errors="coerce") if q1_col in x.columns else np.nan
        ...
        x["forecast_vs_annual_ratio_raw"] = np.where(
            x["base_annual_eps"].notna() & x["base_annual_eps"].ne(0),
            x["forecast_eps_cumulative_raw"] / denom,
            np.nan,
        )
        x["ttm_vs_annual_ratio"] = np.where(
            x["base_annual_eps"].notna() & x["base_annual_eps"].ne(0),
            x["current_eps_ttm"] / denom,
            np.nan,
        )

        x["eps_cumulative_used"] = np.select(
            [actual_complete, forecast_available, ttm_available],
            [x["actual_eps_cumulative"], x["forecast_eps_cumulative"], x["current_eps_ttm"]],
            default=np.nan,
        )
        x["eps_source_used"] = np.select(
            [actual_complete, forecast_available, ttm_available],
            ["ACTUAL_QUARTERLY_EPS", "FORECAST_FROM_REVENUE", "EPS_TTM_PROXY_ONLY"],
            default="EPS_DATA_MISSING",
        )
        x["eps_data_quality"] = np.select(
            [actual_complete, forecast_available, ttm_available],
            ["STRICT", "FORECAST_OR_PROXY", "EPS_TTM_PROXY_ONLY"],
            default="MISSING",
        )

        # legacy 值保留追溯：舊 eps_ratio 其實是 eps_cumulative_used/base_annual_eps，可能混入forecast/TTM。
        x["eps_ratio_legacy"] = np.where(
            x["base_annual_eps"].notna() & x["base_annual_eps"].ne(0),
            x["eps_cumulative_used"] / denom,
            np.nan,
        )
        ...
        # V4.15：舊 forecast_vs_annual_ratio Gate 保留為稽核欄位，不再作為領先選股主Gate。
        x["legacy_forecast_eps_gate"] = x["forecast_vs_annual_ratio"].gt(1.0)
        x["forecast_eps_gate"] = False
        ...
        # V4.13/R1：若 external_valuation 沒 PE，但有價格與基準 EPS，則以 price/base_annual_eps 估算。
        # 注意：fillna 不可傳入 np.where 產生的 ndarray；必須轉成同 index 的 Series，避免 UI 報表背景工作中斷。
        current_price_series = pd.to_numeric(x.get("current_price"), errors="coerce")
        pe_fallback = pd.Series(
            np.where(
                current_price_series.notna() & x["base_annual_eps"].gt(0),
                current_price_series / x["base_annual_eps"],
                np.nan,
            ),
            index=x.index,
            dtype="float64",
        ...
        # V4.15-R2：估值資料缺漏時，不再讓缺值股票取得完整15%估值權重；保留原始估值分數供追溯。
        x["effective_valuation_weight"] = np.where(
            x["valuation_data_missing"],
            0.0,
            HIGH_GROWTH_V412_WEIGHTS["valuation_score_v412"],
        )
        x["effective_valuation_score"] = np.where(
            x["valuation_data_missing"],
            0.0,
            x["valuation_score_v412"],
        )
        x["v412_total_score"] = (
            ...
            x["effective_valuation_score"] * HIGH_GROWTH_V412_WEIGHTS["valuation_score_v412"]
        ).round(2)
        # V4.15-R2：Outlier 只扣 selection_score，不覆蓋 v412_total_score，保留原始模型分數供稽核。
        x["outlier_penalty"] = np.select(
            [
                x["outlier_risk_flag"].isin(["YOY_OUTLIER_CAPPED", "YOY_CAP_HIGH"]),
                x["outlier_risk_flag"].isin(["YOY_NEGATIVE_CAPPED", "YOY_CAP_LOW"]),
                x["forecast_risk_flag"].eq("LOW_BASE_EPS"),
            ],
            [10.0, 8.0, 6.0],
            default=0.0,
        )
        ...
        x["v412_model_definition"] = high_growth_model_definition_text()
        return x
...
class HighGrowthEPSEngine:
    ...
    def _read_sql(self, sql: str, params=None) -> pd.DataFrame:
        return self.revenue_engine._read_sql(sql, params=params)
    ...
    def _table_count(self, table_name: str) -> int:
        ...
        try:
            with self.db.lock:
                row = self.db.conn.cursor().execute(f"SELECT COUNT(*) FROM {table_name}").fetchone()
    ...
    def _apply_formal_block_if_needed(self, x: pd.DataFrame) -> pd.DataFrame:
        """R5C：年度EPS Gate 只限制「正式EPS驗證」層，不可清掉領先預測層。

        設計分層：
        - 02_領先預測通過黑馬池：可使用 EPS_TTM/current_eps_ttm fallback，保留早市場 1~3 季預測功能。
        - 03_EPS_TTM代理追蹤池：保留 proxy 候選，明確標示非正式驗證。
        - 04_正式EPS驗證通過：必須 annual_eps_history / official EPS 完整才允許輸出。

        R5B 問題是把 formal_report_allowed 套到 selection_gate/leading_pass，
        導致 UI 有候選值但 Engine 正式分頁 02 被清成 BLOCKED。
        本函式只阻斷 official/strict 欄位，並新增 eps_basis_type / formal_eps_ready /
        annual_eps_ready / report_status 供報表與 UI 追溯。
        """
        ...
        # R5C：新增分層語意欄位，讓 02/03/UI 可清楚知道資料基準與是否為正式驗證。
        prediction_basis = x.get("prediction_basis", pd.Series("", index=x.index)).fillna("").astype(str)
        selection_source = x.get("selection_eps_source", pd.Series("", index=x.index)).fillna("").astype(str)
        ttm_mask = (
            x.get("ttm_proxy_candidate", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | prediction_basis.str.contains("TTM|EPS_TTM|CURRENT_EPS_TTM", case=False, regex=True)
            | selection_source.str.contains("TTM|EPS_TTM|CURRENT_EPS_TTM", case=False, regex=True)
        )
        ...
        prediction_value_mask = (
            pd.to_numeric(x.get("selection_eps", pd.Series(np.nan, index=x.index)), errors="coerce").gt(0)
            | pd.to_numeric(x.get("predicted_q2_eps", pd.Series(np.nan, index=x.index)), errors="coerce").gt(0)
            | pd.to_numeric(x.get("predicted_q3_eps", pd.Series(np.nan, index=x.index)), errors="coerce").gt(0)
        )
        proxy_mask_current = (
            x.get("proxy_pass", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | x.get("ttm_proxy_candidate", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | ttm_mask
        )
        leading_mask_current = (
            x.get("selection_gate", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | x.get("leading_pass", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | x.get("forecast_candidate", pd.Series(False, index=x.index)).fillna(False).astype(bool)
            | proxy_mask_current
            | prediction_value_mask
        )
        ...
        x["ui_source_type"] = np.select(
            [
                ...
                x.get("ttm_proxy_candidate", pd.Series(False, index=x.index)).fillna(False).astype(bool) | ttm_mask,
                x.get("forecast_candidate", pd.Series(False, index=x.index)).fillna(False).astype(bool),
                prediction_value_mask,
                leading_mask_current,
            ],
            [
                ...
                "EPS_TTM_PROXY",
                "LEADING_FORECAST",
                "PREDICTION_VALUE_CANDIDATE",
                "LEADING_OR_PROXY_CANDIDATE",
            ],
        ...
        # R5C：年度EPS不足時，只阻斷正式驗證/legacy strict，不阻斷領先預測與 selection_gate。
        official_only_cols = [
            "official_eps_verified_pass",
            "official_eps_only_pass",
            "official_eps_revenue_pass",
            "legacy_strict_pass",
            "strict_pass",
        ]
        for c in official_only_cols:
            if c in x.columns:
                x[c] = False
        ...
        if "v412_prediction_quality" in x.columns:
            x["v412_prediction_quality"] = np.where(
                ...
                "LEADING_FORECAST_NOT_FORMAL_ANNUAL_EPS_MISSING",
                x["v412_prediction_quality"],
            )

        if "風險旗標" in x.columns:
            x["風險旗標"] = x["風險旗標"].map(lambda v: self._append_gap_reason_text(v, "需正式EPS驗證"))
        ...
        if "建議動作" in x.columns:
            x["建議動作"] = np.where(
                leading_mask_current,
                "領先預測候選；非正式EPS驗證，待年度EPS補齊後進04正式驗證",
                x["建議動作"],
            )
        return x
    ...
    def _table_cols(self, table_name: str) -> list[str]:
        ...
        try:
            with self.db.lock:
                return [str(r[1]) for r in self.db.conn.cursor().execute(f"PRAGMA table_info({table_name})").fetchall()]
        except Exception:
            return []

    def inspect_db_assets(self) -> pd.DataFrame:
        rows = []
        tables = ["stocks_master", "external_revenue", "external_revenue_history", "external_valuation", "financial_feature_daily", "quarterly_financial_history", "annual_eps_history", "technical_feature_daily", "market_snapshot"]
        for table in tables:
            ...
            rows.append({
                ...
                "V4用途": {
                    "external_revenue": "月營收最新/備援來源",
                    "external_revenue_history": "月營收歷史首選來源，用於季度營收階梯與YoY Gate",
                    "external_valuation": "EPS proxy，不可當嚴格季EPS",
                    "financial_feature_daily": "既有EPS矩陣/每日特徵，不等於正式季EPS",
                    "quarterly_financial_history": "正式季度EPS首選來源",
                    "annual_eps_history": "正式年度EPS首選來源；V4已可自動建表但需資料寫入",
                }.get(table, "輔助/基本資料"),
                "判定": "可用" if exists and count > 0 else ("有表但目前無資料" if exists else "缺表，V4會補schema或需新增資料管線"),
            })
        return pd.DataFrame(rows)

    def _load_master(self) -> pd.DataFrame:
        df = self._read_sql("SELECT stock_id, stock_name, market, industry, theme, sub_theme, is_etf FROM stocks_master WHERE COALESCE(is_active,1)=1")
        if df.empty:
            return pd.DataFrame(columns=["stock_id", "stock_name", "market", "industry", "theme", "sub_theme", "is_etf"])
        df["stock_id"] = df["stock_id"].map(normalize_stock_id)
        df = df[df["stock_id"].astype(str).ne("")].copy()
        df["is_etf"] = pd.to_numeric(df.get("is_etf", 0), errors="coerce").fillna(0).astype(int)
        return df.drop_duplicates("stock_id", keep="first")

    def _load_technical_layer(self) -> pd.DataFrame:
        if not self._table_exists("technical_feature_daily"):
            return pd.DataFrame(columns=["stock_id"])
        q = """
        SELECT t.stock_id, t.close, t.rsi14, t.macd_hist, t.trend_score, t.volume_ratio, t.feature_date
        FROM technical_feature_daily t
        JOIN (SELECT stock_id, MAX(feature_date) AS max_date FROM technical_feature_daily GROUP BY stock_id) m
        ON t.stock_id=m.stock_id AND t.feature_date=m.max_date
        """
        return self._read_sql(q)
    ...
    def build_screening_df(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        x["基本面狀態"] = np.select(
            [x["official_eps_revenue_pass"], x["official_eps_only_pass"], x["forecast_candidate"], x["ttm_proxy_candidate"]],
            ["正式EPS+營收雙驗證通過", "正式EPS本身通過/營收待補", "預測候選", "EPS_TTM追蹤候選"],
            default="未通過/資料不足",
        )
        x["投資分層"] = np.select([
            x["official_eps_revenue_pass"] & pd.to_numeric(x.get("trend_score"), errors="coerce").fillna(0).ge(75) & pd.to_numeric(x.get("rsi14"), errors="coerce").fillna(0).le(75),
            x["official_eps_revenue_pass"],
            x["official_eps_only_pass"],
        ...
        order_cols += ["avg_revenue_yoy", "min_revenue_yoy", "revenue_complete_gate", "revenue_yoy_gate", "revenue_partial_yoy_gate", "revenue_stair_gate", "revenue_strict_gate", "close", "rsi14", "macd_hist", "trend_score", "volume_ratio", "legacy_strict_pass", "strict_pass", "v412_pass", "leading_pass", "legacy_forecast_eps_gate", "forecast_eps_gate", "forecast_candidate", "ttm_proxy_candidate", "proxy_pass", "風險旗標", "建議動作", "資料缺口/未通過原因", "規則版本"]
        order_cols = list(dict.fromkeys(order_cols))
        for c in order_cols:
            if c not in x.columns:
                x[c] = np.nan if c.startswith(("rev_", "yoy_")) or c in ("base_annual_eps", "actual_q1_eps", "actual_eps_cumulative", "selected_actual_eps_cumulative", "current_eps_ttm", "eps_cumulative_used", "forecast_eps_cumulative", "forecast_eps_cumulative_raw", "q1_vs_annual_ratio", "q1_vs_annual_ratio_raw", "selected_actual_vs_annual_ratio", "selected_actual_vs_annual_ratio_raw", "forecast_vs_annual_ratio", "forecast_vs_annual_ratio_raw", "ttm_vs_annual_ratio", "eps_ratio", "eps_ratio_legacy", "selection_score", "selection_eps", "predicted_q2_eps", "predicted_q3_eps", "eps_acceleration", "eps_acceleration_pct", "revenue_acceleration", "pe_ratio", "peg_ratio", "valuation_score_v412", "effective_valuation_score", "effective_valuation_weight", "outlier_penalty", "predicted_q2_eps_score", "eps_acceleration_score_v412", "revenue_acceleration_score_v412", "pe_score", "peg_score", "effective_valuation_score", "effective_valuation_weight", "outlier_penalty", "v412_total_score", "prediction_error", "eps_surprise", "eps_surprise_pct", "avg_revenue_yoy_used") else (False if c in ("official_eps_verified_pass", "official_eps_only_pass", "official_eps_revenue_pass", "legacy_strict_pass", "selection_gate", "valuation_data_missing") else "")
        x = x[order_cols].copy()
    ...
    def _build_revenue_month_coverage(self, required_months: list[str], rev_raw: pd.DataFrame | None = None) -> pd.DataFrame:
        """V4.15-R2：產出要求月份覆蓋表，避免摘要宣稱三個月但實際只有單月資料。"""
        rows = []
        required_months = list(required_months or [])
        rev = rev_raw.copy() if isinstance(rev_raw, pd.DataFrame) else pd.DataFrame()
        if rev is None or rev.empty:
            rev = pd.DataFrame()
            try:
                if self.revenue_engine._table_exists("external_revenue_history"):
                    with self.db.lock:
                        rev = pd.read_sql_query(
                            "SELECT stock_id, revenue_month, revenue, yoy, source_date, update_time FROM external_revenue_history",
                            self.db.conn,
                        )
            except Exception:
                rev = pd.DataFrame()
        available_months_text = ""
        latest_available_month = ""
        if rev is not None and not rev.empty:
            if "revenue_month_norm" not in rev.columns and "revenue_month" in rev.columns:
                rev["revenue_month_norm"] = rev["revenue_month"].map(self.revenue_engine.normalize_month)
            rev["stock_id"] = rev.get("stock_id", "").map(normalize_stock_id) if "stock_id" in rev.columns else ""
            try:
                available_months = sorted([m for m in rev.get("revenue_month_norm", pd.Series(dtype=str)).dropna().astype(str).unique().tolist() if m])
                available_months_text = ",".join(available_months[-12:])
                latest_available_month = available_months[-1] if available_months else ""
            except Exception:
                available_months_text = ""
                latest_available_month = ""
        for m in required_months:
            if rev is not None and not rev.empty and "revenue_month_norm" in rev.columns:
                part = rev[rev["revenue_month_norm"].astype(str).eq(str(m))].copy()
            else:
                part = pd.DataFrame()
    ...
    def _build_same_run_evidence(self, df: pd.DataFrame, revenue_month_coverage: pd.DataFrame) -> pd.DataFrame:
        """V4.15-R2：報表內寫入同一run證據，避免用不同時間log誤判報表。"""
        try:
            master_rows = int(len(self._load_master()))
        except Exception:
            master_rows = -1
        try:
            coverage_missing = ",".join(revenue_month_coverage.loc[revenue_month_coverage["coverage_status"].ne("OK"), "required_month"].astype(str).tolist())
        except Exception:
            coverage_missing = ""
        rows = [
            {"run_evidence_item": "report_generated_at", "run_evidence_value": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
            {"run_evidence_item": "program_rule_version", "run_evidence_value": HIGH_GROWTH_RULE_VERSION},
            {"run_evidence_item": "db_path", "run_evidence_value": str(getattr(self.db, "db_path", ""))},
    ...
    def export_report(self, output_dir: Path | None = None, log_cb=None) -> Path:
        output_dir = Path(output_dir or HIGH_GROWTH_REPORT_DIR)
        output_dir.mkdir(parents=True, exist_ok=True)
        df, assets, rev_raw, eps_features = self.build_screening_df()
        ...
        official_eps_only_count = int(df.get("official_eps_only_pass", pd.Series(False, index=df.index)).fillna(False).astype(bool).sum()) if df is not None and not df.empty else 0
        official_eps_revenue_count = int(df.get("official_eps_revenue_pass", pd.Series(False, index=df.index)).fillna(False).astype(bool).sum()) if df is not None and not df.empty else 0
        legacy_strict_count = int(df.get("legacy_strict_pass", df.get("strict_pass", pd.Series(False, index=df.index))).fillna(False).astype(bool).sum()) if df is not None and not df.empty else 0
        ...
        if not formal_report_allowed:
            # R5D：年度 EPS 斷鏈時，只阻斷 04_正式EPS驗證通過。
            # 02_領先預測通過黑馬池保留 forecast/proxy/value-candidate，避免 UI 有值但 Engine 02 空白。
            official_df = pd.DataFrame([{"狀態": "BLOCKED_BY_ANNUAL_EPS_MISSING", "說明": "annual_eps_history_2025 筆數不足，04正式EPS驗證池不得產出；02領先預測候選仍保留但標示為非正式。"}])
            official_count = official_eps_only_count = official_eps_revenue_count = legacy_strict_count = 0
            try:
                log_info(f"[HIGH_GROWTH][R5D_GATE_SCOPE] formal_allowed=0 official_df=BLOCKED leading_df_retained_rows={len(leading_df)} selection_eps_nonnull={selection_eps_nonnull} predicted_q2_nonnull={predicted_q2_nonnull} predicted_q3_nonnull={predicted_q3_nonnull}")
            except Exception:
                pass
        summary = pd.DataFrame([
            ...
            {"項目": "PE非空筆數", "內容": pe_nonnull_count},
            {"項目": "PEG非空筆數", "內容": peg_nonnull_count},
            {"項目": "有效估值權重非零筆數", "內容": effective_valuation_count},
            {"項目": "strict_pass語意", "內容": "Legacy相容欄位；只代表正式EPS+營收雙驗證，不代表新版領先預測主選股結果。"},
        ...
        validation_kpi = pd.DataFrame([
            {"validation_metric": "formal_report_allowed", "validation_value": int(bool(formal_report_allowed)), "validation_note": "年度EPS基準未建立時只阻斷04正式EPS驗證池，不阻斷02領先預測池。"},
            {"validation_metric": "formal_block_scope_correct", "validation_value": int((not formal_report_allowed) and (official_eps_only_count == 0)), "validation_note": "R5C：formal Gate 只限制 official/strict 欄位；selection_gate/leading_pass/forecast_candidate 不被清空。"},
            {"validation_metric": "leading_pool_retained_when_formal_blocked", "validation_value": int((not formal_report_allowed) and (leading_count > 0 or selection_eps_nonnull == 0)), "validation_note": "02領先預測通過黑馬池保留 forecast/proxy/value-candidate；若 selection_eps/predicted 有值，02 不得因 annual/formal Gate 清空。"},
            {"validation_metric": "annual_eps_history_rows", "validation_value": annual_eps_rows, "validation_note": "2025全年EPS基準筆數；必須>0才可做04正式EPS驗證。"},
        ...
        rules = pd.DataFrame([
            ...
            {"序號": 5, "模組": "Revenue Coverage", "新增/修改": "新增", "程式位置": "HighGrowthEPSEngine._build_revenue_month_coverage", "驗收點": "01A表列出要求月份、可用筆數與缺失月份"},
            {"序號": 6, "模組": "Official EPS Validation", "新增/修改": "修改", "程式位置": "HighGrowthEPSEngine.build_screening_df", "驗收點": "正式EPS本身通過與正式EPS+營收雙驗證拆分，不再只剩單一0"},
        ...
        ui_same_source_df = df.head(300).copy() if df is not None and not df.empty else pd.DataFrame([{"狀態": "NO_UI_SOURCE_ROWS", "說明": "build_screening_df 無資料，UI同源候選池無法輸出。"}])
        revenue_detail_sheet = rev_raw.head(5000) if rev_raw is not None and not rev_raw.empty else pd.DataFrame([
            {
                "diagnostic_item": "NO_REVENUE_DETAIL_ROWS",
                "diagnostic_result": "月營收明細為空；請看01A月份覆蓋與09同Run證據",
                "required_months": ",".join(required_months),
                "missing_required_months": missing_required_months or "NONE",
                "policy": "P1：即使 rev_detail 為空也要輸出診斷，禁止直接消失",
        ...
        tables = {
            ...
            "06_營收明細來源": revenue_detail_sheet,
        ...
        for lang in HIGH_GROWTH_EXPORT_LANGUAGES:
            ...
            localized_tables = {
                high_growth_sheet_label(name, language=lang): localize_high_growth_dataframe_columns(tbl, language=lang) if isinstance(tbl, pd.DataFrame) else tbl
        ...
        quality_msg = f"[HIGH_GROWTH][QUALITY_GATE][R5D] annual_eps_rows={annual_eps_rows}｜official_eps_only={official_eps_only_count}｜official_eps_revenue={official_eps_revenue_count}｜leading={leading_count}｜ttm_proxy={ttm_count}｜missing_months={missing_months_text}｜fail={fail_count}"
    ...
    def get_latest_view(self, limit: int = 300) -> pd.DataFrame:
        df, _, _, _ = self.build_screening_df()
        if df is None or df.empty:
            return pd.DataFrame()
        return df.head(int(limit)).copy()


# Backward compatible alias：舊UI/舊功能名稱仍可使用，但底層已升級到V4。
...
class ExternalDataWriter:
    ...
    def write_result(self, result: ExternalPipelineResult, run_id: str | None = None) -> ExternalPipelineResult:
        ...
        if result.status not in ("success", "fallback") or df.empty:
            ...
            return result
        try:
            # P0：月營收入庫前正規化月份，並拒絕無月份資料，避免覆蓋/污染 history。
            if result.target_table == "external_revenue" and "revenue_month" in df.columns:
                df["revenue_month"] = df["revenue_month"].map(RevenueAccelerationEngine.normalize_month).fillna("").astype(str)
                if "revenue" in df.columns:
                    df["revenue"] = pd.to_numeric(df["revenue"], errors="coerce")
                df = df[(df["revenue_month"].astype(str).ne("")) & (df.get("revenue", pd.Series(dtype=float)) > 0)].copy()
                if df.empty:
                    result.status = "fail"
                    result.data_ready = 0
            ...
            with self.db.lock:
                if result.target_table == "market_snapshot":
                    ...
                    # R3：月營收成功寫入 external_revenue 時，同步累積到 external_revenue_history。
                    # 原先只更新快照表，HighGrowth EPS 主要讀 history，導致連結改成功但 required months 仍命中 0。
                    if result.target_table == "external_revenue" and {"stock_id", "revenue_month"}.issubset(df.columns):
                        hist = df.copy()
                        if "source_url" not in hist.columns:
                            hist["source_url"] = str(result.request_url or "")
                        keep_hist = ["stock_id", "revenue_month", "revenue", "mom", "yoy", "cumulative_revenue", "cumulative_yoy", "source_url", "source_date", "update_time"]
                        for c in keep_hist:
                            if c not in hist.columns:
                                hist[c] = None
                        hist["revenue"] = pd.to_numeric(hist["revenue"], errors="coerce")
                        hist = hist[(hist["revenue_month"].astype(str).ne("")) & (hist["revenue"] > 0)].copy()
                        hist = hist[keep_hist].drop_duplicates(subset=["stock_id", "revenue_month"], keep="last")
                        cur = self.db.conn.cursor()
                        cache_mask = hist["source_url"].fillna("").astype(str).str.contains("OFFICIAL_OPENAPI_CACHE", na=False)
                        hist_regular = hist.loc[~cache_mask].copy()
                        hist_cache = hist.loc[cache_mask].copy()
                        if not hist_regular.empty:
                            cur.executemany(
                                "DELETE FROM external_revenue_history WHERE stock_id=? AND revenue_month=?",
                                hist_regular[["stock_id", "revenue_month"]].astype(str).drop_duplicates().itertuples(index=False, name=None),
                            )
                            hist_regular.to_sql("external_revenue_history", self.db.conn, if_exists="append", index=False)
                        if not hist_cache.empty:
                            # R4A：官方快取只補缺，禁止覆蓋既有有效 history。
                            cur.executemany(
                                """INSERT OR IGNORE INTO external_revenue_history
                                   (stock_id, revenue_month, revenue, mom, yoy, cumulative_revenue, cumulative_yoy, source_url, source_date, update_time)
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                                hist_cache[keep_hist].itertuples(index=False, name=None),
                            )
                            log_info(f"[REVENUE][OPENAPI_CACHE] history append-only inserted_or_ignored rows={len(hist_cache)}")
                else:
...
class ExternalDataFetcher:
    ...
    def _normalize_revenue_month(self, value) -> str:
        ...
        norm = RevenueAccelerationEngine.normalize_month(value)
        if norm:
            return norm
        # 不用目前月份硬補，避免抓到無月份欄位時把全部資料寫成當月，污染 required_months。
        return ""

    def _normalize_revenue_month_series(self, s: pd.Series) -> pd.Series:
        """P0：所有 revenue_month 入庫/讀取前一律正規化，支援既有 11504 舊資料。"""
        if s is None:
            return pd.Series(dtype="object")
        return pd.Series(s).map(self._normalize_revenue_month).fillna("").astype(str)

    def _read_cached_revenue_fallback(self, reason: str = "") -> pd.DataFrame:
        """P0：官方 OpenAPI 當次失敗時，回退使用最後成功月營收快取，不清空 external_revenue/history。

        回傳 latest revenue_month 的每股一筆資料，並明確標示 source_url/source_level，
        讓 FinancialFeatureEngine 與 HighGrowth 報表仍可使用 202604 等已成功月份。
        """
        parts = []
        for table in ["external_revenue_history", "external_revenue"]:
            try:
                if not hasattr(self.db, "read_table"):
                    continue
                df = self.db.read_table(table, limit=None)
                if df is None or df.empty or "revenue_month" not in df.columns or "stock_id" not in df.columns:
                    continue
                x = df.copy()
                x["stock_id"] = x["stock_id"].map(normalize_stock_id)
                x["revenue_month"] = self._normalize_revenue_month_series(x["revenue_month"])
                x = x[(x["stock_id"].astype(str) != "") & (x["revenue_month"].astype(str) != "")].copy()
                if x.empty:
                    continue
                for c in ["revenue", "mom", "yoy", "cumulative_revenue", "cumulative_yoy"]:
                    if c not in x.columns:
                ...
                if "update_time" not in x.columns:
                    x["update_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                x["source_url"] = str(x.get("source_url", "")) if "source_url" not in x.columns else x["source_url"].fillna("").astype(str)
                x["_source_table"] = table
                parts.append(x[["stock_id", "revenue_month", "revenue", "mom", "yoy", "cumulative_revenue", "cumulative_yoy", "source_date", "source_url", "update_time", "_source_table"]])
            except Exception as exc:
                log_warning(f"[REVENUE][CACHE_FALLBACK] read {table} failed｜{exc}")
        if not parts:
            return pd.DataFrame()
        all_df = pd.concat(parts, ignore_index=True)
        all_df = all_df[all_df["revenue_month"].astype(str).str.fullmatch(r"\d{6}", na=False)].copy()
        if all_df.empty:
            return pd.DataFrame()
        latest_month = sorted(all_df["revenue_month"].astype(str).unique().tolist())[-1]
        out = all_df[all_df["revenue_month"].astype(str).eq(latest_month)].copy()
        out = out.sort_values(["stock_id", "_source_table"]).drop_duplicates(subset=["stock_id", "revenue_month"], keep="first")
        out["source_url"] = "local_cache:external_revenue_history_or_external_revenue"
        out["source_date"] = out["source_date"].replace("", datetime.now().strftime("%Y-%m-%d"))
        out["update_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_warning(f"[REVENUE][CACHE_FALLBACK] official fetch failed; preserve latest cached month={latest_month} rows={len(out)} reason={reason}")
    ...
    def _parse_revenue_rows(self, raw: pd.DataFrame, source_label: str = "") -> tuple[pd.DataFrame, str]:
        ...
        for _, r in raw.iterrows():
            ...
            if not sid:
                continue
            month = self._normalize_revenue_month(r.get(col_map.get("revenue_month"), ""))
            if not month:
                continue
    ...
    def fetch_revenue(self, module: str, cfg: dict, run_id: str) -> ExternalPipelineResult:
        ...
        for url in [u for u in urls if str(u or "").strip()]:
            try:
                ...
                if "t187ap05_R" in url:
                    label = "TPEX_REVENUE_R"
                else:
        ...
        if not df.empty:
            df["revenue_month"] = self._normalize_revenue_month_series(df["revenue_month"])
            if "revenue" in df.columns:
                df["revenue"] = pd.to_numeric(df["revenue"], errors="coerce")
            df = df[(df["revenue_month"].astype(str).ne("")) & (df["revenue"] > 0)].drop_duplicates(subset=["stock_id", "revenue_month"], keep="last")

            # R4A：只新增官方 OpenAPI cache artifact，不改 R4 原本成功資料流。
            cache_downloader = RevenueOpenAPICacheDownloader(REVENUE_CACHE_DIR)
            cache_path = cache_downloader.save_cache_df(df, source_tag="r4_original_success")

            # R4A：若 R4 成功結果仍缺高成長 EPS 指定月份，才用本地/下載快取補缺；只補不存在的 stock_id+month。
            required_months = []
            try:
            ...
            if missing_months:
                ...
                if supplement is not None and not supplement.empty:
                    ...
                    log_warning(f"[REVENUE][OPENAPI_CACHE] R4 success but missing months={missing_months}; append-only supplement rows={len(supplement)} cache={cache_path}")
                else:
                    log_warning(f"[REVENUE][OPENAPI_CACHE] R4 success but missing months={missing_months}; cache supplement rows=0, keep R4 result unchanged")
        ...
        # R4A：官方來源完全失敗時，先嘗試官方 OpenAPI cache downloader；失敗後才回到 R4 local history fallback。
        cache_downloader = RevenueOpenAPICacheDownloader(REVENUE_CACHE_DIR)
        official_cache_df, official_cache_path, cache_errors = cache_downloader.download_latest_cache(include_optional=False)
        if official_cache_df is not None and not official_cache_df.empty:
            return ExternalPipelineResult(
                module,
                "fallback",
                df=official_cache_df,
                source_name=cfg.get("source_name","") + "｜official openapi cache append-only",
                official_url=cfg.get("official_url",""),
                request_url=" | ".join(urls) + f" | OFFICIAL_OPENAPI_CACHE:{official_cache_path}",
                source_date=str(official_cache_df["source_date"].dropna().astype(str).max() or datetime.now().strftime("%Y-%m-%d")),
                http_status="official_fetch_fail_openapi_cache_fallback",
                fallback_count=1,
                error_message="官方月營收 OpenAPI 當次失敗，改用 official openapi cache downloader；" + (" ; ".join(errors) or cache_errors or "官方月營收 OpenAPI 資料抓取失敗"),
                target_table=cfg.get("target_table",""),
                data_ready=1,
                source_level="official_openapi_cache_append_only",
            )

        # P0/R4：當次官方來源與快取下載器都失敗時，不讓 revenue_rows=0 污染 EPS Matrix；保留最後成功月份。
        cache_df = self._read_cached_revenue_fallback(reason=" ; ".join(errors) + (" ; " + cache_errors if cache_errors else ""))
        if cache_df is not None and not cache_df.empty:
            return ExternalPipelineResult(
                module,
                "fallback",
                df=cache_df,
                source_name=cfg.get("source_name","") + "｜local cache fallback",
                official_url=cfg.get("official_url",""),
                request_url=" | ".join(urls) + " | local_cache:external_revenue_history",
                source_date=str(cache_df["source_date"].dropna().astype(str).max() or datetime.now().strftime("%Y-%m-%d")),
                http_status="official_fail_cache_fallback",
                fallback_count=1,
                error_message="官方月營收 OpenAPI 當次失敗，已保留最後成功快取月份；" + (" ; ".join(errors) or "官方月營收 OpenAPI 資料抓取失敗"),
                target_table=cfg.get("target_table",""),
                data_ready=1,
                source_level="local_cache_fallback_preserve_history",
            )
    ...
    def _execute_module(self, module: str, cfg: dict, run_id: str) -> ExternalPipelineResult:
        ...
        # P0：防止任何舊 mopsfin CSV 退役網址從其他同步路徑漏進來。
        if module == "revenue":
            all_urls = " ".join([str(cfg.get("request_template", ""))] + [str(u) for u in (cfg.get("request_templates") or [])])
            retired_host = "mopsfin" + ".twse.com.tw"
            retired_o = "t187ap05_" + "O.csv"
            retired_l = "t187ap05_" + "L.csv"
            if retired_host in all_urls or retired_o in all_urls or retired_l in all_urls:
                log_warning(f"[REVENUE][OLD_URL_GUARD] deprecated revenue URL detected in config and ignored: {all_urls}")
...
class FinancialFeatureEngine:
    ...
    def _latest_revenue_by_stock(self) -> pd.DataFrame:
        """P0：合併 external_revenue_history + external_revenue，月份正規化後取最後成功月份。"""
        parts = []
        for table, priority in [("external_revenue_history", 1), ("external_revenue", 2)]:
            try:
                df = self.db.read_table(table, limit=None)
                x = self._standardize_feature_source(df, "external_revenue", "revenue_month")
                if x is None or x.empty:
                    continue
                x["revenue_month"] = x["revenue_month"].map(RevenueAccelerationEngine.normalize_month).fillna("").astype(str)
                x = x[x["revenue_month"].astype(str).str.fullmatch(r"\d{6}", na=False)].copy()
                if x.empty:
                    continue
                ...
                log_warning(f"[EPS MATRIX][REVENUE_HISTORY_FALLBACK] skip {table}｜{exc}")
        if not parts:
            return pd.DataFrame()
        all_df = pd.concat(parts, ignore_index=True)
    ...
    def build_feature_batch(self, run_id: str | None = None, write_db: bool = True, log_limit: int = 10) -> pd.DataFrame:
        ...
        # P0：FinancialFeature 不只讀當次快照 external_revenue；若同步失敗，改用 history 最後成功月份。
        revenue = self._latest_revenue_by_stock()
...
class MasterTradingEngine:
    ...
    def get_trade_pool(self, filtered_df: pd.DataFrame, progress_cb=None, log_cb=None, cancel_cb=None) -> dict:
        ...
        # FIX9：TOP20 保留原「主流TOP20 + 起爆TOP20 + 雙引擎共振」池化設計，
        # 但不得在 AI TOP20 入口重建 EPS Matrix；EPS/financial_feature_daily 應由每日增量更新完成。
        # 此處只讀既有快取，避免 TOP20/投資組合引擎誤觸發全市場重型重算。
        if log_cb:
            try:
                feature_rows = self.db.get_latest_financial_features()
                log_cb(f"[EPS MATRIX][READ-ONLY] AI選股只讀 financial_feature_daily cache：{0 if feature_rows is None else len(feature_rows)} 筆")
            except Exception as exc:
                log_cb(f"[EPS MATRIX][READ-ONLY][WARN] financial_feature_daily 讀取失敗：{exc}")
...
class AppUI:
    def __init__(self, root, db: DBManager):
        ...
        self.last_ai_portfolio_tables = {}
        self.last_ai_portfolio_df = pd.DataFrame()
        self.last_prebreakout_report_path = None
        self.last_prebreakout_df = pd.DataFrame()
        self.last_highgrowth_report_path = None
        self.last_highgrowth_df = pd.DataFrame()
        self.ai_portfolio_vars = {}
        self.ai_portfolio_quota_vars = {}
        self.ai_portfolio_weight_vars = {}
    ...
    def _selected_highgrowth_quarters(self) -> list[str]:
        try:
            selected = []
            for q in ["Q1", "Q2", "Q3", "Q4"]:
                var = getattr(self, f"highgrowth_{q.lower()}_var", None)
                if var is not None and bool(var.get()):
                    selected.append(q)
            return normalize_high_growth_quarters(selected)
        except Exception:
            return list(HIGH_GROWTH_DEFAULT_QUARTERS)
    ...
    def _build_highgrowth_tab(self):
        ...
        self.highgrowth_q3_var = tk.BooleanVar(value=False)
    ...
    def _rebuild_highgrowth_tab_headers(self):
        """V4.15：高成長EPS UI固定中文；此函式只負責重套中文欄位標題。"""
        try:
            ...
            if hasattr(self, "highgrowth_status_var"):
                self.highgrowth_status_var.set(f"V4.15報表輸出：{HIGH_GROWTH_REPORT_DIR}｜Excel中文/英文各一份")
        except Exception as exc:
    ...
    def on_prebreakout_after_market_build(self):
        """UI按鈕：執行爆發前盤後DB建立。"""
        def worker():
            self.ui_call(self.start_task, "爆發前盤後DB建立", 3)
            result = run_prebreakout_after_market_builder(db=self.db, log_cb=lambda m: self.ui_call(self.append_log, m))
            self.ui_call(self.update_task, "爆發前盤後DB建立", 3, 3, success=int(result.get("signal_rows", 0)), item="Signal完成")
            self.ui_call(self.append_log, f"爆發前盤後DB建立完成｜{result}")
            df = PreBreakoutIntegratedEngine(self.db).get_latest_signals()
            self.ui_call(self._refresh_prebreakout_tree, df)
            self.ui_call(self.finish_task, "爆發前盤後DB建立", f"完成：Signal {int(result.get('signal_rows', 0))} 筆")
        self._run_in_thread(worker, "prebreakout_after_market_builder")

    def on_prebreakout_premarket_report(self):
        """UI按鈕：產生 reports/prebreakout_premarket_YYYYMMDD.xlsx。"""
        def worker():
            self.ui_call(self.start_task, "爆發前盤前報表", 2)
            path = run_prebreakout_premarket_analyzer(db=self.db, output_dir=PREBREAKOUT_REPORT_DIR, log_cb=lambda m: self.ui_call(self.append_log, m))
            self.last_prebreakout_report_path = Path(path)
            df = PreBreakoutIntegratedEngine(self.db).get_latest_signals()
            self.ui_call(self._refresh_prebreakout_tree, df)
            self.ui_call(self.update_task, "爆發前盤前報表", 2, 2, success=len(df) if isinstance(df, pd.DataFrame) else 0, item="Excel完成")
            self.ui_call(self.append_log, f"爆發前盤前報表已產生：{path}")
            self.ui_call(messagebox.showinfo, "完成", f"已產生爆發前盤前報表：\n{path}")
            self.ui_call(self.finish_task, "爆發前盤前報表", f"已產生：{path}")
        self._run_in_thread(worker, "prebreakout_premarket_report")

    def on_open_prebreakout_report(self):
        """UI按鈕：開啟最新爆發前報表。"""
    ...
    def _build_ai_portfolio_controls(self):
        """FIX9：建立獨立 AI投資組合引擎控制區；不影響 TOP20。"""
        control = ttk.LabelFrame(self.tab_ai_portfolio, text="AI投資組合引擎｜勾選池別 / 配額 / 權重（不取代TOP20）", padding=8)
        control.pack(fill="x", padx=4, pady=4)
        cfg = self.load_ai_portfolio_config()
        pools = list(DEFAULT_AI_PORTFOLIO_CONFIG.keys())
        for idx, pool in enumerate(pools):
            row = idx // 4
            base_col = (idx % 4) * 4
            item = cfg.get(pool, DEFAULT_AI_PORTFOLIO_CONFIG.get(pool, {}))
            enabled_var = tk.BooleanVar(value=bool(item.get("enabled", True)))
            quota_var = tk.IntVar(value=int(item.get("quota", 3) or 3))
            weight_var = tk.DoubleVar(value=float(item.get("weight", 1.0) or 1.0))
            self.ai_portfolio_vars[pool] = enabled_var
            self.ai_portfolio_quota_vars[pool] = quota_var
            self.ai_portfolio_weight_vars[pool] = weight_var
            ttk.Checkbutton(control, text=pool, variable=enabled_var).grid(row=row, column=base_col, sticky="w", padx=(2, 2), pady=2)
            ttk.Label(control, text="配額").grid(row=row, column=base_col + 1, sticky="e")
            ttk.Spinbox(control, from_=0, to=20, width=4, textvariable=quota_var).grid(row=row, column=base_col + 2, sticky="w")
            ttk.Entry(control, textvariable=weight_var, width=5).grid(row=row, column=base_col + 3, sticky="w", padx=(2, 10))
        btn_row = ttk.Frame(control)
        btn_row.grid(row=3, column=0, columnspan=16, sticky="w", pady=(6, 0))
        ttk.Button(btn_row, text="產出AI投資組合", command=self.show_ai_portfolio_engine).pack(side="left", padx=4)
        ttk.Button(btn_row, text="儲存組合設定", command=self.save_ai_portfolio_config_from_ui).pack(side="left", padx=4)
        ttk.Button(btn_row, text="匯出AI投資組合報表", command=self.export_ai_portfolio_report).pack(side="left", padx=4)
        ttk.Label(btn_row, text="說明：TOP20仍走原短線主攻雙引擎；本頁只做獨立投資組合分池。").pack(side="left", padx=12)

    def load_ai_portfolio_config(self) -> dict:
        cfg = json.loads(json.dumps(DEFAULT_AI_PORTFOLIO_CONFIG, ensure_ascii=False))
        try:
            if AI_PORTFOLIO_CONFIG_PATH.exists():
                raw = json.loads(AI_PORTFOLIO_CONFIG_PATH.read_text(encoding="utf-8"))
                if isinstance(raw, dict):
                    for pool, item in raw.items():
                        if pool in cfg and isinstance(item, dict):
                            cfg[pool].update(item)
        except Exception as exc:
            log_warning(f"[AI_PORTFOLIO] pool_config 讀取失敗，改用預設：{exc}")
        return cfg

    def save_ai_portfolio_config_from_ui(self):
        try:
            cfg = self._get_ai_portfolio_config_from_ui()
            AI_PORTFOLIO_CONFIG_PATH.write_text(json.dumps(cfg, ensure_ascii=False, indent=2), encoding="utf-8")
            self.append_log(f"[AI_PORTFOLIO] 組合設定已儲存：{AI_PORTFOLIO_CONFIG_PATH}")
            messagebox.showinfo("AI投資組合", f"組合設定已儲存：\n{AI_PORTFOLIO_CONFIG_PATH}")
        except Exception as exc:
            messagebox.showerror("AI投資組合", f"儲存組合設定失敗：{exc}")

    def _get_ai_portfolio_config_from_ui(self) -> dict:
        cfg = {}
        for pool, default in DEFAULT_AI_PORTFOLIO_CONFIG.items():
            enabled = bool(self.ai_portfolio_vars.get(pool, tk.BooleanVar(value=default.get("enabled", True))).get()) if self.ai_portfolio_vars else bool(default.get("enabled", True))
            try:
                quota = int(self.ai_portfolio_quota_vars.get(pool).get()) if pool in self.ai_portfolio_quota_vars else int(default.get("quota", 3))
            except Exception:
                quota = int(default.get("quota", 3))
            try:
                weight = float(self.ai_portfolio_weight_vars.get(pool).get()) if pool in self.ai_portfolio_weight_vars else float(default.get("weight", 1.0))
            except Exception:
                weight = float(default.get("weight", 1.0))
            cfg[pool] = {"enabled": enabled, "quota": max(0, quota), "weight": weight}
        return cfg

    def _ai_portfolio_base_df(self) -> pd.DataFrame:
        ranking = self.db.get_latest_ranking()
        if ranking is None or ranking.empty:
            return pd.DataFrame()
        x = ranking.copy()
        try:
            ...
            pass
        for c in ["stock_id", "stock_name", "market", "industry", "theme", "signal", "action"]:
            if c not in x.columns:
                x[c] = ""
            x[c] = x[c].fillna("").astype(str)
        for c in ["total_score", "ai_score", "momentum_score", "trend_score", "volume_score", "risk_score", "financial_score", "revenue_eps_score", "revenue_yoy", "eps_yoy", "pe", "pb", "dividend_yield", "rr"]:
            if c not in x.columns:
                x[c] = np.nan
            x[c] = pd.to_numeric(x[c], errors="coerce")
    ...
    def _ai_portfolio_pool_reason(self, pool_name: str) -> str:
        mapping = {
            "市場主流池": "題材/產業均分位於前段，代表當前資金主流。",
            "未來科技池": "題材/產業/名稱命中AI、ASIC、CPO、網通、散熱、電源等未來科技關鍵字。",
            "營收成長池": "營收YoY、財務分數或AI分數具成長動能；用營收先行替代落後EPS。",
            "價值池": "低PE/低PB/殖利率或財務分數較佳，偏穩健價值。",
            "波段攻擊池": "總分、AI、趨勢、量能或技術訊號偏強，適合波段觀察。",
            "老師低接池": "訊號/建議偏低接、拉回、整理偏多，適合等支撐。",
            "ETF防守池": "ETF/防守資產獨立分流，不混入短線TOP20。",
            "排除風險池": "弱勢、風險分偏低、總分偏低或建議減碼/觀望。",
        }
        return mapping.get(pool_name, "依池別規則入選。")

    def build_ai_portfolio_tables(self, config: dict | None = None) -> dict:
        config = config or self._get_ai_portfolio_config_from_ui()
        base = self._ai_portfolio_base_df()
        if base is None or base.empty:
            return {"01_AI投資組合總表": pd.DataFrame(), "10_組合設定與查檢表": pd.DataFrame([{"項目": "狀態", "內容": "ranking_result 無資料"}])}
        stock_base = base[~base["is_etf_pool"]].copy()
        etf_base = base[base["is_etf_pool"]].copy()
        theme_score = stock_base.groupby("theme", as_index=False).agg(theme_avg_total=("total_score", "mean"), theme_avg_ai=("ai_score", "mean"), theme_count=("stock_id", "count")) if not stock_base.empty else pd.DataFrame()
        if not theme_score.empty:
            theme_score["theme_rank_score"] = theme_score["theme_avg_total"].fillna(0) * 0.65 + theme_score["theme_avg_ai"].fillna(0) * 0.35
            hot_cut = theme_score["theme_rank_score"].quantile(0.80)
            hot_themes = set(theme_score[theme_score["theme_rank_score"] >= hot_cut]["theme"].astype(str).tolist())
        else:
            hot_themes = set()
        future_pat = "|".join([re.escape(k) for k in AI_PORTFOLIO_FUTURE_TECH_KEYWORDS])
        pools = {}
        pools["02_市場主流池"] = stock_base[stock_base["theme"].isin(hot_themes)].copy() if hot_themes else stock_base.sort_values(["total_score", "ai_score"], ascending=False).head(30).copy()
        pools["03_未來科技池"] = stock_base[(stock_base["theme"].str.contains(future_pat, case=False, regex=True)) | (stock_base["industry"].str.contains(future_pat, case=False, regex=True)) | (stock_base["stock_name"].str.contains(future_pat, case=False, regex=True))].copy()
        pools["04_營收成長池"] = stock_base[((stock_base["revenue_yoy"].fillna(-999) >= 20) | (stock_base["revenue_eps_score"].fillna(0) >= 65) | ((stock_base["ai_score"].fillna(0) >= 70) & (stock_base["total_score"].fillna(0) >= 55)))].copy()
        pools["05_價值池"] = stock_base[(((stock_base["pe"].fillna(999) > 0) & (stock_base["pe"].fillna(999) <= 20)) | ((stock_base["pb"].fillna(999) > 0) & (stock_base["pb"].fillna(999) <= 2.5)) | (stock_base["dividend_yield"].fillna(0) >= 3) | (stock_base["financial_score"].fillna(0) >= 70))].copy()
        pools["06_波段攻擊池"] = stock_base[((stock_base["total_score"].fillna(0) >= 65) | (stock_base["ai_score"].fillna(0) >= 70) | (stock_base["trend_score"].fillna(0) >= 70) | (stock_base["volume_score"].fillna(0) >= 70) | (stock_base["signal"].str.contains("強勢|突破|偏多", regex=True)))].copy()
        pools["07_老師低接池"] = stock_base[(stock_base["action"].str.contains("低接|拉回|等待|區間", regex=True)) | (stock_base["signal"].str.contains("整理偏多|區間整理", regex=True))].copy()
        pools["08_ETF防守池"] = etf_base.copy()
        pools["09_排除風險池"] = base[((base["risk_score"].fillna(100) <= 35) | (base["total_score"].fillna(0) < 45) | (base["signal"].str.contains("轉弱|急跌|風險", regex=True)) | (base["action"].str.contains("減碼|觀望|防守", regex=True)))].copy()
        sheet_to_pool = {
            "02_市場主流池": "市場主流池", "03_未來科技池": "未來科技池", "04_營收成長池": "營收成長池", "05_價值池": "價值池",
            "06_波段攻擊池": "波段攻擊池", "07_老師低接池": "老師低接池", "08_ETF防守池": "ETF防守池", "09_排除風險池": "排除風險池",
        }
        out_tables = {}
        combined = []
        ...
        for sheet_name, dfp in pools.items():
            ...
            x = dfp.copy()
            if not x.empty:
                x["portfolio_pool"] = pool_name
                x["pool_quota"] = quota
                x["pool_weight"] = weight
                x["pool_reason"] = self._ai_portfolio_pool_reason(pool_name)
        ...
        if combined:
            ...
            combo["source_pool_count"] = combo.groupby("stock_id")["portfolio_pool"].transform("nunique")
            combo["source_pools"] = combo.groupby("stock_id")["portfolio_pool"].transform(lambda s: "、".join(sorted(set(s.astype(str)))))
            combo = combo.sort_values(["source_pool_count", "portfolio_score", "total_score", "ai_score"], ascending=False).drop_duplicates("stock_id", keep="first").head(20).copy()
    ...
    def _populate_ai_portfolio_tree(self, df: pd.DataFrame):
        try:
            self.ai_portfolio_tree.delete(*self.ai_portfolio_tree.get_children())
            if df is None or df.empty:
                return
            for _, r in df.iterrows():
    ...
    def show_ai_portfolio_engine(self):
        ...
        def worker():
            try:
                ...
                self.ui_call(self.start_task, "AI投資組合引擎", 3)
                cfg = self._get_ai_portfolio_config_from_ui()
                AI_PORTFOLIO_CONFIG_PATH.write_text(json.dumps(cfg, ensure_ascii=False, indent=2), encoding="utf-8")
                self.ui_call(self.update_task, "AI投資組合引擎", 1, 3, item="建立分池")
                tables = self.build_ai_portfolio_tables(cfg)
                main = tables.get("01_AI投資組合總表", pd.DataFrame())
                self.last_ai_portfolio_tables = tables
                self.last_ai_portfolio_df = main
        ...
        self._run_in_thread(worker, "ai_portfolio_engine")

    def export_ai_portfolio_report(self):
        def worker():
            try:
                cfg = self._get_ai_portfolio_config_from_ui()
                tables = self.build_ai_portfolio_tables(cfg)
                self.last_ai_portfolio_tables = tables
                self.last_ai_portfolio_df = tables.get("01_AI投資組合總表", pd.DataFrame())
                out_base = RUNTIME_DIR / f"AI_Portfolio_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                out_path, out_type = write_table_bundle(out_base, tables, preferred="excel")
                ...
                self.ui_call(messagebox.showerror, "AI投資組合", f"匯出AI投資組合失敗：\n{exc}")
        self._run_in_thread(worker, "export_ai_portfolio")
    ...
    def build_report_usage_sheet(self) -> pd.DataFrame:
        rows = [
            ...
            {"報表": "Trade_TOP20", "用途": "候選20，進第二層分析。原TOP20保留主流TOP20+起爆TOP20+雙引擎共振，不被AI投資組合取代。"},
            {"報表": "AI投資組合總表", "用途": "使用者勾選池別、配額、權重後產出的策略組合；獨立於TOP20。"},
    ...
    def export_analysis_excel(self):
        ...
        def worker():
            try:
                ...
                ai_tables = getattr(self, "last_ai_portfolio_tables", {}) or {}
                for _ai_sheet in ["01_AI投資組合總表", "02_市場主流池", "03_未來科技池", "04_營收成長池", "05_價值池", "06_波段攻擊池", "07_老師低接池", "08_ETF防守池", "09_排除風險池", "10_組合設定與查檢表"]:
                    _ai_df = ai_tables.get(_ai_sheet, pd.DataFrame()) if isinstance(ai_tables, dict) else pd.DataFrame()
                    if _ai_df is not None and not _ai_df.empty:
                        tables[_ai_sheet] = _ai_df
